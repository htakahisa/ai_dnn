...
11111111111111111000000000000000000000000001
11111111111111110001111110111110111111000111
11111100000000000011111110111110000111002221
...
def parse_fixed_maze(maze_str):
    lines = [line.strip() for line in maze_str.strip("\n").split("\n") if line.strip()]
    raw = np.array([[int(ch) for ch in line] for line in lines], dtype=np.int32)
    grid = np.where(raw == 1, 1, 0).astype(np.int32)  # 2は通路扱い
    goal_candidates = [tuple(x) for x in zip(*np.where(raw == 2))]
    return grid, goal_candidates


class GridWorldEnv(gym.Env):
    def __init__(self, fixed_grid=None, goal_candidates=None, wall_ratio=0.10):
        ...
        # 観測: [player_r, player_c, goal_r, goal_c, up, down, left, right, last_action(4次元one-hot), neighbor_dist(4次元)] (16次元)
        self.obs_dim = 16
        ...
        self.goal_pos = self.goal_candidates[0] if self.goal_candidates else \
                         np.array([self.height - 1, self.width - 1], dtype=np.int32)
            ...
            for r in range(self.height):
                for c in range(self.width):
                    ...
                    if np.random.rand() < self.wall_ratio:
                        self.grid[r, c] = 1
            if self._is_reachable(np.array([0, 0]), np.array([self.height - 1, self.width - 1])):
                break
    ...
    def reset(self, seed=None, options=None):
        while True:
            ...
            if self.goal_candidates is not None:
                idx = np.random.randint(len(self.goal_candidates))
                self.goal_pos = self.goal_candidates[idx]
            else:
        ...
        self.current_step = 0
        self.last_action = None
        self.dist_map = self._bfs_distances_from_goal()
...
if __name__ == "__main__":
    ...
    print(f"grid shape: {fixed_grid.shape}, ゴール候補数: {len(goal_candidates)}")
    ...
    writer.close()
    torch.save(policy_net.state_dict(), "dqn_gridworld_fixedmap.pt")