...
DEFENDER_ROLE_CYCLE = ["フラッシュ", "スモーカー", "シーカー", "フラッシュ", "スモーカー"]
...
class SimChar:
    def __init__(self, name, team, pos, role=None):
        self.name = name
        self.team = team
        self.pos = list(pos)
        self.is_alive = True
...
class RetakeEnv:
    ...
    def reset(self):
        ...
        defender_spawns = pick_spawns(DEFENDER_SPAWNS if DEFENDER_SPAWNS else WALKABLE, 5)
        hold_candidates = [p for p in self.site_zone if GRID[p[0], p[1]] != 2 and p not in used]
        attacker_spawns = pick_spawns(hold_candidates if hold_candidates else WALKABLE, 5)

        self.chars = []
        for i, pos in enumerate(attacker_spawns):
            self.chars.append(SimChar(f"Attacker{i+1}", "A", pos, role=None))
        for i, pos in enumerate(defender_spawns):
            self.chars.append(
                SimChar(f"Defender{i+1}", "D", pos, role=DEFENDER_ROLE_CYCLE[i % len(DEFENDER_ROLE_CYCLE)])
            )
