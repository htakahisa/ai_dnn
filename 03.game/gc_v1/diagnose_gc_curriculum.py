...
def main():
    ...
    parser.add_argument(
        "--seeds", type=int, nargs="+", default=(3026091700, 5026091700, 6026091700)
    )
    ...
    sources = {
        p: args.models_dir / f"dqn_attacker_{p}_gc_{args.kind}.pt"
        for p in trainer.PHASES
    }
    source_hashes = {
        p: hashlib.sha256(s.read_bytes()).hexdigest() for p, s in sources.items()
    }
    checkpoints = {
    ...
    escort_state = checkpoints["escort"].get("model_state_dict", checkpoints["escort"])
    escort_actions = int(escort_state["advantage_head.2.weight"].shape[0])
    guard_checkpoint = checkpoints["guard"]
    policies = {
        "carry": trainer.runtime.AttackerCarryDuelingDQN(
            obs_dim=checkpoints["carry"]["obs_dim"]
        ),
        "escort": trainer.escort_runtime.DuelingQNetwork(
            checkpoints["escort"]["obs_dim"], escort_actions
        ),
        "guard": trainer.guard_runtime.AttackerGuardDuelingDQN(
            obs_dim=guard_checkpoint["obs_dim"],
            action_dim=guard_checkpoint.get(
                "action_dim", trainer.guard_runtime.ACTION_DIM
            ),
        ),
    }
    for p in trainer.PHASES:
        policies[p].load_state_dict(
            checkpoints[p].get("model_state_dict", checkpoints[p])
        )
    ...
    for checkpoint in checkpoints.values():
        ...
        if expected is not None and expected != macro_hash:
            raise ValueError(
                "loaded Macro weights differ from the checkpoint's training Macro"
            )
    rows, trace = [], []
    selected = {}
    ...
    def traced_move(char):
        ...
        for action, (dr, dc) in zip((2, 4, 6, 8), CARDINAL):
            ...
            blockers = [
                dict(name=c.name, team=c.team, pos=tuple(c.pos))
                for c in game.chars
        ...
        screen = carrier_screening_status(
            game,
            char,
            game.chars,
        ...
        designated_pos = (
            tuple(map(int, designated.pos)) if designated is not None else None
        )
        designated_engaged = bool(
            designated is not None and can_engage(game, designated, game.chars)
        )
        ally_positions = {
        ...
        for neighbor in neighbors:
            cell = neighbor["cell"]
            inside = (
                0 <= cell[0] < game.grid.shape[0] and 0 <= cell[1] < game.grid.shape[1]
            )
            neighbor["distance"] = (
                int(distance[cell]) if inside and distance is not None else None
            )
            neighbor["wall"] = not inside or game.grid[cell] == 1
        improving = [
            n
            for n in neighbors
            if n["distance"] is not None and 0 <= n["distance"] < int(distance[pos])
        ]
        legal_progress = [
            n for n in improving if decision.get("mask", [False] * 11)[n["action"]]
        ]
        trace.append(
            dict(
                tick=game.battle_tick,
                name=char.name,
                before=pos,
                after=after,
                goal=goal,
                ...
                final_distance=(
                    int(session.distances(target)[after]) if target else None
                ),
                neighbors=neighbors,
                legal_progress=legal_progress,
                engaged=fighting,
                planting=char.plant_timer,
                ...
                designated_chebyshev=(
                    max(
                        abs(designated_pos[0] - pos[0]), abs(designated_pos[1] - pos[1])
                    )
                    if designated_pos is not None
                    else None
                ),
    ...
    for seed in args.seeds:
        for i in range(args.episodes):
            ...
            summary = dict(
                ...
                at_waypoint=sum(
                    t["distance_after"] is not None and t["distance_after"] <= 1
                    for t in trace
                ),
                quiet_stays=sum(
                    t["before"] == t["after"] and not t["engaged"] and not t["planting"]
                    for t in trace
                ),
                reversals=sum(
                    trace[j]["after"] == trace[j - 2]["after"]
                    and trace[j]["after"] != trace[j - 1]["after"]
                    for j in range(2, len(trace))
                ),
                goal_changes=sum(
                    t.get("decision_goal") != trace[j - 1].get("decision_goal")
                    for j, t in enumerate(trace)
                    if j
                ),
                quiet_stays_with_progress=sum(
                    t["before"] == t["after"]
                    and not t["engaged"]
                    and not t["planting"]
                    and bool(t["legal_progress"])
                    for t in trace
                ),
                blocked_moves=sum(
                    t.get("action") in (2, 4, 6, 8) and t["before"] == t["after"]
                    for t in trace
                ),
                blocked_by_ally=int(
                    sum(
                        any(b["team"] == "A" for b in n["blockers"])
                        for t in trace
                        for n in t["neighbors"]
                        if n["action"] == t.get("action")
                        and t.get("action") in (2, 4, 6, 8)
                    )
                ),
                screen_ready_ticks=int(
                    sum(t.get("screen_ready", False) for t in trace)
                ),
                designated_far_ticks=int(
                    sum((t.get("designated_chebyshev") or 0) > 6 for t in trace)
                ),
    ...
    if any(
        hashlib.sha256(s.read_bytes()).hexdigest() != source_hashes[p]
        for p, s in sources.items()
    ):
    ...
    args.json_output.write_text(
        json.dumps(
            dict(
                macro_source=dict(path=str(macro_path), sha256=macro_hash),
                grid_shape=game.grid.shape,
                models={
                    p: dict(
                        path=str(sources[p]),
                        episode=checkpoints[p].get("episode"),
                        sha256=source_hashes[p],
                        positioning_version=checkpoints[p].get("positioning_version"),
                    )
                    for p in trainer.PHASES
                },
                episodes=rows,
            ...
            default=lambda value: (
                value.item() if isinstance(value, np.generic) else list(value)
            ),
        ),
        encoding="utf-8",
    )
