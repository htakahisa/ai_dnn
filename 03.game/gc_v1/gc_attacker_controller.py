...
from learning_attacker_carry_gc import LearningAttackerCarryGCController
from learning_attacker_escort_gc import LearningAttackerEscortGCController
from learning_attacker_retrieve_gc import LearningAttackerRetrieveGCController
from learning_attacker_guard_gc import LearningAttackerGuardGCController


class GCAttackerController(BaseController):
    ...
    def __init__(
        self,
        carry_model_path="data/attacker_carry_gc_data/dqn_attacker_carry_gc_best_by_eval.pt",
        escort_model_path="data/attacker_escort_gc_data/dqn_attacker_escort_gc_best_by_eval.pt",
        retrieve_model_path="data/attacker_retrieve_gc_data/dqn_attacker_retrieve_gc_best_by_eval.pt",
        guard_model_path="data/attacker_guard_gc_data/dqn_attacker_guard_gc_best_by_eval.pt",
        greedy=False,
    ):
        super().__init__()
        self.carry_controller = LearningAttackerCarryGCController(model_path=carry_model_path, greedy=greedy)
        self.escort_controller = LearningAttackerEscortGCController(model_path=escort_model_path, greedy=greedy)
        self.retrieve_controller = LearningAttackerRetrieveGCController(model_path=retrieve_model_path, greedy=greedy)
        self.guard_controller = LearningAttackerGuardGCController(model_path=guard_model_path, greedy=greedy)
...
# 旧run_game設定との互換用
MultiRoleAttackerController = GCAttackerController
