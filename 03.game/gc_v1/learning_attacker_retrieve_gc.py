...
1人」をstickyに選ぶ(learning_attacker_guard_gc.pyのチーム内選出と
...
# ---------------------------------------------------------------------------
# Dueling DQN (gc_v1/train_attacker_retrieve.py と同一構造。
...
# 壁のみを考慮する(スモークによる遮蔽は考慮しない。他のgc_v1推論
# コントローラーと同じ簡略化方針)。
# ---------------------------------------------------------------------------
...
# 推論コントローラー
# ---------------------------------------------------------------------------
class LearningAttackerRetrieveGCController:
    """gc_v1固定チーム専用、落下スパイク回収フェーズ(retrieve)のAttacker AI。
    ...
    def __init__(self, model_path=DEFAULT_MODEL_PATH, greedy=True, verbose=False):
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
                ...
                if verbose:
                    ...
                    print(
                        f"[LearningAttackerRetrieveGCController] loaded: {model_path} "
        ...
        self._debug_log_path = "attacker_retrieve_gc_debug.log"
    ...
    def decide_move(self, char, game_state):
        ...
        # (learning_attacker_guard_gc.py の fallback方針と同様)。
        if visible_enemies:
            nearest = min(
                visible_enemies,
                key=lambda e: max(
