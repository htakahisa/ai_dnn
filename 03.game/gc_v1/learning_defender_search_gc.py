...
gc_v1/train_defender_search.py と同じ優先順位ツリーを踏襲する:
...
def _bfs_distance_map_multi(grid, goals):
    """候補群のどこか最寄りまでのマルチソースBFS距離マップ。"""
    height, width = grid.shape
    dist = np.full((height, width), -1, dtype=np.int32)
    queue = deque()
...
def _find_marker_positions(grid, value):
    ...
    if not hits:
        raise RuntimeError(
            f"map_data_search_gc.py に値{value}の防衛候補マスがありません。"
        )
    return hits
...
# 推論コントローラー
# ---------------------------------------------------------------------------
class LearningDefenderSearchGCController:
    """gc_v1固定チーム専用、プラント前フェーズのDefender AI。
