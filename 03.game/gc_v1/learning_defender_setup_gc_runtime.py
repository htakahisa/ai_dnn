...
OPPONENT_NAMES = sorted(
    name for name in all_preset_names()
...
class LearningDefenderSetupGCRuntime:
    def __init__(self, model_path=None, *, device=None, verbose=True):
        ...
        self.variation_name = SETUP_VARIATIONS[0]
        ...
        if self.verbose:
            print(
                ...
                f"variations={SETUP_VARIATIONS}"
            )

    def set_game(self, game):
        self.game = game

    def set_context(self, opponent_name=None, variation=None):
        if opponent_name is not None:
            self.opponent_name = str(opponent_name)
        if variation is not None:
            if isinstance(variation, str):
                if variation not in SETUP_VARIATIONS:
                    raise ValueError(f"Unknown Setup variation: {variation}")
                self.variation_index = SETUP_VARIATIONS.index(variation)
            else:
                self.variation_index = int(variation)
            if not (0 <= self.variation_index < VARIATION_DIM):
                raise ValueError(f"Setup variation index out of range: {self.variation_index}")
            self.variation_name = SETUP_VARIATIONS[self.variation_index]
