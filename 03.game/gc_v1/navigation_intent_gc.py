...
# The exact designated screener starts earlier than the legacy formation
# features.  Keeping the old bound intact avoids changing older policies.
SCREEN_COMMITMENT_MAX_FINAL_DISTANCE = 40
    ...
    if goal not in cache:
        ...
        while queue:
            ...
            for dr, dc in CARDINAL:
                ...
                if (
                    0 <= nxt[0] < distance.shape[0]
                    and 0 <= nxt[1] < distance.shape[1]
                    and game.grid[nxt] != 1
                    and distance[nxt] < 0
                ):
...
def navigation_intent(game, char):
    ...
    if getattr(char, "has_spike", False) and plant is not None:
        goal = plant
    elif goal is None:
        goal = plant
    if goal is not None:
        ...
        grid = game.grid
        if (
            not (0 <= goal[0] < grid.shape[0] and 0 <= goal[1] < grid.shape[1])
            or grid[goal] == 1
        ):
            goal = plant
        ...
        # task is the registered own-team plant target on the same site.
            ...
            getattr(char, "has_spike", False) and plant is not None and grid[goal] == 2
        ):
...
def can_engage(game, char, chars):
    ...
    for enemy in chars:
        ...
        if any(
            other.name not in (char.name, enemy.name)
            and getattr(other, "is_alive", True)
            and tuple(other.pos) in intermediate
            for other in chars
        ):
            continue
...
def navigation_context_features(game, char, chars, distance, history, goal):
    ...
    for i, (dr, dc) in enumerate(CARDINAL):
        nxt = (pos[0] + dr, pos[1] + dc)
        wall = (
            not (0 <= nxt[0] < grid.shape[0] and 0 <= nxt[1] < grid.shape[1])
            or grid[nxt] == 1
        )
        features[4 + i] = wall
    ...
    features[16] = np.clip(
        (remaining - max(0, current) - PLANT_REQUIRED_TICKS) / sum(grid.shape), -1, 1
    )
...
def carrier_screening_status(game, carrier, chars, cache):
    ...
    allies = [
        ...
        and c.team == carrier.team
        and getattr(c, "is_alive", True)
    ]
    near = [
        c
        for c in allies
        if max(abs(c.pos[0] - carrier_pos[0]), abs(c.pos[1] - carrier_pos[1])) <= 4
    ]
    ahead = [
        c
        for c in allies
    ...
    def same_route(ally):
        ...
        if strategy == "DEFAULT":
            return (
                carrier_role in {"MAIN_LEAD", "DEFAULT_MAIN"}
                and role in {"MAIN_LEAD", "DEFAULT_MAIN"}
                and side == carrier_side
            )
        return side == carrier_side and role not in {
            "FAKE_SELL",
            "FAKE_WAIT",
            "SUPPORT",
            "OPPOSITE_SCOUT",
    ...
    for _ in range(FORMATION_LEAD_STEPS):
        ...
        for dr, dc in CARDINAL:
            ...
            if (
                0 <= nxt[0] < route_distance.shape[0]
                and 0 <= nxt[1] < route_distance.shape[1]
    ...
    designated = next(
        (
            ally
            for ally in reachable
            if saved_key == designation_key and ally.name == saved_name
        ),
        None,
    )
    if designated is None:
        designated = min(
            reachable,
            key=lambda ally: (
                int(target_distance[tuple(map(int, ally.pos))]),
                ally.name,
            ),
            default=None,
        )
        if env is not None:
            ...
            env._gc_screening_designated_name = (
                designated.name if designated is not None else None
            )
    designated_distance = (
        -1
        if designated is None
        else int(target_distance[tuple(map(int, designated.pos))])
    )
    designated_route_distance = (
        -1
        if designated is None
        else int(route_distance[tuple(map(int, designated.pos))])
    )
    screen_ready = bool(
        designated is not None
        and designated_distance <= FORMATION_READY_RADIUS
        and current >= 0
        and 0 <= designated_route_distance <= current - 2
    )
    return {
        ...
        "formation_target_distance": target_distance,
        "formation_candidates": candidates,
        "designated": designated,
        "designated_distance": designated_distance,
...
def carrier_formation_features(game, char, chars, cache, escort=False):
    ...
    if escort:
        ...
        features[2] = (
            max(0, status["designated_distance"]) / norm if features[0] else 0.0
        )
        features[3] = float(
            any(c.name == char.name for c in status["formation_candidates"])
        )
    else:
...
def designated_route_blocking(status, carrier):
    ...
    adjacent = (
        max(
            abs(designated_pos[0] - carrier_pos[0]),
            abs(designated_pos[1] - carrier_pos[1]),
        )
        == 1
    )
...
def carrier_route_clearance_features(game, char, chars, cache):
    ...
    for i, (dr, dc) in enumerate(CARDINAL):
        ...
        if not legal:
            continue
        still_adjacent = (
            max(abs(nxt[0] - carrier_pos[0]), abs(nxt[1] - carrier_pos[1])) == 1
        )
...
def carrier_safe_screen_advance_features(game, char, chars, cache):
    ...
    for i, (dr, dc) in enumerate(CARDINAL):
        ...
        if (
            not legal
            or current_target < 0
            or not 0 <= int(target_distance[nxt]) < current_target
        ):
            continue
...
def carrier_screen_commitment_features(game, char, chars, cache):
    """Preferred U/D/L/R screen step plus active/ready state.

    Choose one collision-safe shortest step toward the exact formation target.
    Once ready, keep moving the screen forward whenever Carrier closes the
    three-cell lead instead of treating the first arrival as a permanent hold.
    """
    features = np.zeros(SCREEN_COMMITMENT_DIM, dtype=np.float32)
    carrier = next(
        (
            c
            for c in chars
    ...
    current_target = int(target_distance[pos])
    carrier_route = int(route_distance[carrier_pos])
    current_route = int(route_distance[pos])
    occupied = {
        tuple(map(int, other.pos))
        for other in chars
        if other.name != char.name and getattr(other, "is_alive", True)
    ...
    for action, (dr, dc) in enumerate(CARDINAL):
        ...
        if not legal:
            continue
        nxt_target = int(target_distance[nxt])
        nxt_route = int(route_distance[nxt])
        if status["screen_ready"]:
            # A ready screener must keep a small lead as Carrier advances.
            # Once the lead is large enough, ready-without-direction means hold.
            route_lead = carrier_route - current_route
            if route_lead > FORMATION_LEAD_STEPS or not 0 <= nxt_route < current_route:
                continue
        elif current_target < 0 or not 0 <= nxt_target < current_target:
            continue
        adjacent = max(abs(nxt[0] - carrier_pos[0]), abs(nxt[1] - carrier_pos[1])) == 1
        blocks_route = (
            adjacent and carrier_route >= 0 and 0 <= nxt_route < carrier_route
        )
        if not blocks_route:
            candidates.append(
                (nxt_target, nxt_route if nxt_route >= 0 else 10**6, nxt, action)
            )
    if candidates:
        features[min(candidates)[3]] = 1.0
        features[4] = 1.0
    if status["screen_ready"]:
        features[5] = 1.0
    return features
