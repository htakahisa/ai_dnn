param(
    [string]$OutputDir = (Join-Path $PSScriptRoot "data\attacker_gc_curriculum_v9")
)

# v8's evaluation selector retained its episode 0 baseline.  Start from that
# synchronized Carry/Escort/Guard set instead of the degraded episode 750 set.
$gcInitDir = Join-Path $PSScriptRoot "data\attacker_gc_curriculum_v8_duration15_10"
...
if ([int]$gcBundle.episode -ne 0) {
    throw "Expected the preserved v8 episode 0 checkpoint, got episode $($gcBundle.episode)"
}
...
& "D:\git\python\python.exe" -X utf8 $gcTrainer `
    ...
    --episodes 2000 --curriculum-episodes 1000 --navigation-bootstrap-episodes 1000 `
    --navigation-demo-weight 2.0 --navigation-retention-weight 0.50 --n-step 5 --final-mix 0.20 `
    --start-stats 50 10 0 --final-stats 100 60 40 --seed 2026095000 `
