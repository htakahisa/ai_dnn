param(
    [string]$OutputDir = (Join-Path $PSScriptRoot "data\attacker_gc_curriculum_v10")
)

# v9 learned Escort formation but did not improve outcomes.  Its selector kept
# the synchronized episode 0 baseline, which is the safe starting point here.
$gcInitDir = Join-Path $PSScriptRoot "data\attacker_gc_curriculum_v9"
...
if ([int]$gcBundle.episode -ne 0) {
    throw "Expected the preserved v9 episode 0 checkpoint, got episode $($gcBundle.episode)"
}
...
& "D:\git\python\python.exe" -X utf8 $gcTrainer `
    ...
    --navigation-demo-weight 1.5 --navigation-retention-weight 0.35 --n-step 5 --final-mix 0.20 `
    --start-stats 50 10 0 --final-stats 100 60 40 --seed 2026096000 `
