...
from train_attacker_carry_gc_real import (
    ROOT, Pending, RealCarrySession, expanded_state, optimize, runtime,
)
...
from navigation_intent_gc import (navigation_intent, can_engage,
...
VERSIONS = {"carry": 9, "escort": 8, "guard": 3}
TRAINING_REVISION = "entry_support_ultimates_v15"


def expand_policy_state(checkpoint, obs_dim, action_dim=None):
    state = expanded_state(checkpoint) if "model_state_dict" in checkpoint else dict(checkpoint)
    state = dict(state)
    weights = state["feature.0.weight"]
    if weights.shape[1] > obs_dim:
        raise ValueError("source observation dimension is larger than the requested model")
    if weights.shape[1] < obs_dim:
    ...
    if action_dim is not None:
        ...
        if out_weight.shape[0] > action_dim:
            raise ValueError("source action dimension is larger than the requested model")
        if out_weight.shape[0] < action_dim:
...
class ProgressTracker:
    ...
    def step(self, name, goal, before, after, engaged, required=True):
        ...
        idle = 0 if improvement or engaged or not required else self.idle.get(name, 0) + 1
        self.idle[name] = idle
...
def combat_reward(phase, action, engaged):
    if not engaged:
        return 0.0
    stationary = action in (escort_runtime.ACTION_STAY, escort_runtime.ACTION_ABILITY) if phase == "escort" else action == 0 or action % 2 == 1
...
def navigation_teacher_action(phase, obs, mask, final_approach=False):
    ...
    if phase == "guard":
        return None
    old_dim = runtime.TACTICAL_OBS_DIM if phase == "carry" else escort_runtime.TACTICAL_OBS_DIM
    if len(obs) < old_dim + 18 or obs[old_dim - 1] or obs[13 if phase == "carry" else 24]:
        return None
    ...
    if at_waypoint and not (phase == "carry" and final_approach and obs[25] > 0):
        return stay if mask[stay] else None
    actions = (2, 4, 6, 8) if phase == "carry" else (0, 1, 2, 3)
    candidates = [(float(obs[old_dim + i]), a) for i, a in enumerate(actions) if mask[a]]
    if candidates:
...
def observable_teacher_action(phase, obs, mask, controller, context, view=None):
    ...
    if (ultimate_action < len(mask) and mask[ultimate_action]
            and ultimate_teacher_needed(phase, char, state, view)):
    ...
    if (phase == "escort" and len(obs) >= escort_runtime.DIRECTIONAL_CLEARANCE_OBS_DIM
            and escort_blocks_route):
        clearance = obs[escort_runtime.CLEARANCE_OBS_DIM:
                        escort_runtime.DIRECTIONAL_CLEARANCE_OBS_DIM]
        moves = [(float(obs[escort_runtime.TACTICAL_OBS_DIM + action]), action)
                 for action in range(4) if clearance[action] > 0 and mask[action]]
        if moves:
            return max(moves, key=lambda pair: pair[0])[1]
    if phase == "escort" and len(obs) >= escort_runtime.ENTRY_SUPPORT_OBS_DIM:
        safe_advance = obs[escort_runtime.DIRECTIONAL_CLEARANCE_OBS_DIM:
                           escort_runtime.ENTRY_SUPPORT_OBS_DIM]
        moves = [(float(obs[escort_runtime.TACTICAL_OBS_DIM + action]), action)
                 for action in range(4) if safe_advance[action] > 0 and mask[action]]
        if moves:
            return max(moves, key=lambda pair: pair[0])[1]
    if (phase == "escort" and len(obs) >= escort_runtime.NAVIGATION_OBS_DIM
        ...
        moves = [(float(obs[escort_runtime.TACTICAL_OBS_DIM + i]), i)
    ...
    if carrier is not None:
        ...
        if FORMATION_MIN_FINAL_DISTANCE < final_distance <= FORMATION_MAX_FINAL_DISTANCE:
            designated = screen["designated"]
            if (phase == "carry" and carrier.name == char.name
                    and carrier_entry_sync_needed(screen, state, obs)):
                stay = 0
                return stay if mask[stay] else None
            if phase == "escort" and designated is not None and designated.name == char.name:
    ...
    final = phase == "carry" and navigation_intent(view, char)[0] == team_plant_target(view)
    return navigation_teacher_action(phase, obs, mask, final_approach=final)


def ultimate_teacher_needed(phase, char, state, view):
    ...
    if getattr(char, "ultimate_points", 0) < getattr(char, "ultimate_cost", 1):
        return False
    if phase == "guard":
        active_defuse = any(timer > 0 for timer, _required in
                            (state.get("defender_defuse_info") or {}).values())
    ...
    return bool(designated is not None and designated.name == char.name
...
def carrier_entry_sync_needed(screen, state, obs=None):
    ...
    idle_feature = (float(obs[runtime.TACTICAL_OBS_DIM + 14])
                    if obs is not None and len(obs) >= runtime.NAVIGATION_OBS_DIM else 0.0)
    return bool(
...
def optimize_demonstrations(policy, optimizer, samples, weight, batch_size=64):
    ...
    loss = weight * (torch.max(competitor + margin, dim=1).values
                     - q.gather(1, indices[:, None]).squeeze(1)).mean()
    optimizer.zero_grad()
    loss.backward()
...
def selection_score(metrics, max_no_entry, max_timeout):
    ...
    carrier_death = metrics.get("worst_no_entry_carrier_death_rate",
                                metrics.get("no_entry_carrier_death_rate", 1.0))
    return (int(violation == 0), -violation, -carrier_death, metrics["worst_round_win_rate"],
            metrics["round_win_rate"], -metrics["carry_spawn_tick_rate"])


def entry_quality_passed(metrics, max_no_entry, max_timeout):
    return (metrics.get("worst_carry_no_entry_rate", metrics["carry_no_entry_rate"]) <= max_no_entry
...
class CurriculumSession(RealCarrySession):
    def __init__(self, sources, policies, gamma):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            inner.guard = guard_runtime.LearningAttackerGuardGCController(str(sources["guard"]))
        self.controllers = {"carry": inner.carry, "escort": inner.escort, "guard": inner.guard}
        self.actor = None
    ...
    def selector(self, phase):
        def choose(obs, mask):
            ...
            teacher = (observable_teacher_action(phase, obs, mask, self.controllers[phase],
                                                 getattr(self, "decision_context", None), view=view)
                       if getattr(self, "collect_demonstrations", False) else None)
            if teacher is not None:
                self.demonstrations[phase].append((obs.copy(), teacher, mask.copy()))
            if (teacher is not None and phase not in getattr(self, "frozen_phases", ())
                    and random.random() < self.teacher_probability):
                action = teacher
            elif random.random() < self.epsilon:
                action = int(random.choice(valid)) if len(valid) else (4 if phase == "escort" else 0)
            else:
                with torch.no_grad():
                    q = self.policies[phase](torch.from_numpy(obs).unsqueeze(0)).squeeze(0).numpy()
            ...
            if getattr(self, "decision_context", None) is not None and phase != "guard":
                ...
                self.action_goals[key] = navigation_intent(self.controllers[phase].game, char)[0]
            return action
        return choose
    ...
    def flush(self, key, pending, obs=None, mask=None, terminal=True):
        ...
        if obs is None:
            ...
            mask = np.ones(self.policies[phase].advantage_head[-1].out_features, dtype=bool)
        self.transitions[phase].append((name, (pending.obs, pending.action, pending.reward,
                                              obs.copy(), mask.copy(), terminal, pending.ticks)))
    ...
    def play(self, seed, stats, epsilon=0.0, training=True, teacher_probability=0.0):
        ...
        for char in game.chars:
            if char.team == "D":
                char.accuracy, char.hs_rate, char.dodge_rate = (x / 100.0 for x in stats)
        ...
        def support_counts(carrier, carrier_pos, goal, positions):
            allies = [c for c in game.chars if c.team == "A" and c.name != carrier.name and c.is_alive]
            near = sum(max(abs(positions[c.name][0] - carrier_pos[0]),
                           abs(positions[c.name][1] - carrier_pos[1])) <= 4 for c in allies)
            ahead = 0
            if goal is not None:
                distance = self.distances(goal)
                carrier_distance = int(distance[carrier_pos])
                if carrier_distance >= 0:
                    ahead = sum(0 <= int(distance[positions[c.name]]) < carrier_distance
                                and max(abs(positions[c.name][0] - carrier_pos[0]),
                                        abs(positions[c.name][1] - carrier_pos[1])) <= 8
                                for c in allies)
            return near, ahead

        while not game.round_over:
            planted_before = game.is_planted
            holder = next((c for c in game.chars if c.team == "A" and c.is_alive and c.has_spike), None)
            holder_pos = tuple(holder.pos) if holder else None
            pre_screen_status = (carrier_screening_status(game, holder, game.chars, self.maps)
            ...
            carry_pending = (self.pending.get(("carry", holder.name))
                             if holder is not None else None)
            pre_sync_needed = bool(
                pre_screen_status is not None
                and carrier_entry_sync_needed(
                    pre_screen_status, {"round_timer": game.round_timer},
                    carry_pending.obs if carry_pending is not None else None))
            before = {c.name: (tuple(c.pos), c.hp, c.plant_timer, c.kills, c.is_alive)
                      for c in game.chars}
            engaged = {c.name: can_engage(game, c, game.chars) for c in game.chars if c.team == "A" and c.is_alive}
            goals = {}
            ...
            if (holder is not None and holder.is_alive and pre_sync_needed
            ...
            if holder is not None and holder.is_alive:
                screen_status = carrier_screening_status(game, holder, game.chars, self.maps)
                screening_names = {c.name for c in screen_status["formation_candidates"]}
                ...
                if (FORMATION_MIN_FINAL_DISTANCE < screen_status["final_distance"]
                        <= FORMATION_MAX_FINAL_DISTANCE and screening_names):
                    ...
                    if screen_status["final_distance"] <= 8:
                        ...
                        entry_formation_ready_opportunities += int(screen_status["screen_ready"])
            ...
            if holder is not None and entry_tick is None:
                ...
                near, ahead = support_counts(holder, carrier_pos, route_goal,
                                             {name: values[0] for name, values in before.items()})
                took_damage = holder.hp < before[holder.name][1]
                if first_carrier_threat_tick is None and (engaged.get(holder.name, False) or took_damage):
                ...
                if before[holder.name][4] and not holder.is_alive:
                    ...
                    carrier_preentry_death_engaged = bool(engaged.get(holder.name, False))
            ...
            for name, goal in self.controllers["guard"]._assigned_guard_positions.items():
                goals.setdefault(name, tuple(goal))
            chars = {c.name: c for c in game.chars}
            next_holder = next((c for c in game.chars if c.team == "A" and c.is_alive and c.has_spike), None)
            for key in list(self.pending):
                phase, name = key
                char = chars[name]
                pos, hp, progress, kills, alive = before[name]
                reward = -0.05 - 0.025 * max(0, hp - char.hp) + 0.5 * max(0, char.kills - kills)
                ...
                if phase == "carry":
                    ...
                    if route_goal is not None:
                        ...
                        if distance[pos] >= 0 and distance[tuple(char.pos)] >= 0:
                            reward += self.progress.step(name, route_goal, int(distance[pos]), int(distance[tuple(char.pos)]), fighting,
                                                         required=int(distance[tuple(char.pos)]) > 1 and char.plant_timer == 0)
                    carry_ticks += 1
                    recent = carry_positions.setdefault(name, deque(maxlen=3))
                    if not recent:
                        recent.append(pos)
                    recent.append(tuple(char.pos))
                    reversed_move = len(recent) == 3 and recent[0] == recent[2] and recent[0] != recent[1]
                    carry_reversals += int(reversed_move)
                    quiet_stall = tuple(char.pos) == pos and not fighting and char.plant_timer == 0 and not new_plant
                    carry_quiet_stalls += int(quiet_stall)
                    if reversed_move and not fighting:
                        reward -= 0.15
                    spawn_distance = self.distances(source_positions[name])[tuple(char.pos)]
                    carry_spawn_ticks += int(0 <= spawn_distance <= 6)
                    if 0 <= spawn_distance <= 6 and game.battle_tick > 20 and not fighting:
                        # Waiting briefly for information is valid. Spending
                        # the attack clock at spawn without a duel is not.
                        reward -= 0.15
                    if name == getattr(holder, "name", None) and pre_sync_needed and not fighting:
                        if tuple(char.pos) == pos:
                            reward += 0.30
                        elif route_goal is not None and distance[tuple(char.pos)] < distance[pos]:
                            reward -= 0.30
                        else:
                            reward -= 0.10
                    ...
                    if holder is not None and not fighting and holder.is_alive:
                        ...
                        if carry_goal is not None:
                            ...
                            if (max(abs(escort_cell[0] - holder_cell[0]), abs(escort_cell[1] - holder_cell[1])) == 1
                                    and 0 <= carry_distance[escort_cell] < carry_distance[holder_cell]):
                    ...
                    if pre_designated and pre_route_blocking and not fighting:
                        ...
                        if holder is not None and holder.is_alive and moved and not post_blocking:
                            reward += 0.75
                            designated_route_clear_moves += 1
                        elif holder is not None and holder.is_alive and not moved:
                            reward -= 0.75
                    if formation_active and not route_blocking and not fighting:
                        # Measure both cells against the same post-move target.
                        # The carrier may also move during this tick, so comparing
                        # two separately computed targets teaches the wrong sign.
                        formation_distance = screen_status["formation_target_distance"]
                        ...
                        if before_distance >= 0 and after_distance >= 0:
                            reward += 0.25 * max(0, before_distance - after_distance)
                            reward -= 0.15 * max(0, after_distance - before_distance)
                        reward += 0.18 if screen_status["screen_ready"] else -0.05
                elif phase == "guard" and name in goals:
                    goal = goals[name]
                    distance = self.distances(goal)
                    if distance[pos] >= 0 and distance[tuple(char.pos)] >= 0:
                        reward += self.progress.step(name, goal, int(distance[pos]), int(distance[tuple(char.pos)]), fighting,
        ...
        for phase, rows in self.transitions.items():
            ...
            for index in last.values():
                ...
                rows[index] = (name, (obs, action, reward + (20.0 if won else -20.0),
                                     next_obs, mask, terminal, ticks))
        no_entry = entry_tick is None and not game.is_planted
        team_wiped = not any(c.team == "A" and c.is_alive for c in game.chars)
        return {"attacker_win": won, "planted": bool(game.is_planted),
                "registered": bool(game.is_planted and tuple(game.planted_pos) in REGISTERED_PLANT_CELLS),
                "ticks": game.battle_tick, "guard_ticks": guard_ticks,
                "guard_position_ticks": guard_arrivals, "opponent_stats": list(stats),
                "combat_ticks": combat_ticks, "combat_stop_ticks": combat_stops,
                "carry_ticks": carry_ticks, "carry_spawn_ticks": carry_spawn_ticks,
                "carry_reversals": carry_reversals, "carry_quiet_stalls": carry_quiet_stalls,
                "carry_entry_tick": entry_tick, "plant_tick": plant_tick,
                ...
                "no_entry_other": bool(no_entry and not carrier_preentry_death
                                       and not team_wiped and game.round_timer > 0),
...
def evaluate(session, episodes, seed, final):
    ...
    return {"episodes": episodes, "seed": seed, "opponent_stats": list(final),
            ...
            "carry_spawn_tick_rate": sum(r.get("carry_spawn_ticks", 0) for r in rows) / max(1, sum(r.get("carry_ticks", 0) for r in rows)),
            "carry_reversal_tick_rate": sum(r.get("carry_reversals", 0) for r in rows) / max(1, sum(r.get("carry_ticks", 0) for r in rows)),
            "carry_quiet_stall_tick_rate": sum(r.get("carry_quiet_stalls", 0) for r in rows) / max(1, sum(r.get("carry_ticks", 0) for r in rows)),
            "timeout_rate": sum(r.get("timed_out", False) for r in rows) / episodes,
            "carrier_preentry_death_rate": sum(r.get("carrier_preentry_death", False) for r in rows) / episodes,
            "no_entry_carrier_death_rate": sum(r.get("no_entry_carrier_death", False) for r in rows) / episodes,
            "no_entry_team_wipe_rate": sum(r.get("no_entry_team_wipe", False) for r in rows) / episodes,
            "no_entry_other_rate": sum(r.get("no_entry_other", False) for r in rows) / episodes,
            "mean_death_support_near": float(np.mean([r["carrier_preentry_death_support_near"] for r in death_rows])) if death_rows else 0.0,
            "mean_death_support_ahead": float(np.mean([r["carrier_preentry_death_support_ahead"] for r in death_rows])) if death_rows else 0.0,
            "mean_first_threat_support_near": float(np.mean([r["first_carrier_threat_support_near"] for r in threat_rows])) if threat_rows else 0.0,
            "mean_first_threat_support_ahead": float(np.mean([r["first_carrier_threat_support_ahead"] for r in threat_rows])) if threat_rows else 0.0,
            "death_screen_ready_rate": sum(r.get("carrier_preentry_death_screen_ready", False) for r in death_rows) / max(1, len(death_rows)),
            "first_threat_screen_ready_rate": sum(r.get("first_carrier_threat_screen_ready", False) for r in threat_rows) / max(1, len(threat_rows)),
            "screened_approach_rate": sum(r.get("screened_approach_success_ticks", 0) for r in rows) /
                                      max(1, sum(r.get("screened_approach_ticks", 0) for r in rows)),
            "screened_approach_ticks": sum(r.get("screened_approach_ticks", 0) for r in rows),
            "screened_approach_success_ticks": sum(r.get("screened_approach_success_ticks", 0) for r in rows),
            "formation_ready_approach_rate": sum(r.get("formation_ready_ticks", 0) for r in rows) /
                                             max(1, sum(r.get("formation_approach_ticks", 0) for r in rows)),
            "formation_approach_ticks": sum(r.get("formation_approach_ticks", 0) for r in rows),
            "formation_ready_ticks": sum(r.get("formation_ready_ticks", 0) for r in rows),
            "entry_formation_ready_rate": sum(r.get("entry_formation_ready_ticks", 0) for r in rows) /
            ...
            "entry_formation_ready_ticks": sum(r.get("entry_formation_ready_ticks", 0) for r in rows),
            ...
            "designated_route_block_rate": sum(r.get("designated_route_block_ticks", 0) for r in rows) /
                                             max(1, sum(r.get("formation_approach_ticks", 0) for r in rows)),
            "designated_route_block_ticks": sum(r.get("designated_route_block_ticks", 0) for r in rows),
            "designated_route_clear_rate": sum(r.get("designated_route_clear_moves", 0) for r in rows) /
                                             max(1, sum(r.get("designated_route_clear_opportunities", 0) for r in rows)),
            "designated_route_clear_opportunities": sum(r.get("designated_route_clear_opportunities", 0) for r in rows),
            "designated_route_clear_moves": sum(r.get("designated_route_clear_moves", 0) for r in rows),
            "combat_stop_rate": sum(r.get("combat_stop_ticks", 0) for r in rows) / max(1, sum(r.get("combat_ticks", 0) for r in rows)),
            "postplant_win_rate": sum(r.get("postplant_win", False) for r in rows) / max(1, sum(r["planted"] for r in rows))}
...
def evaluate_multi(session, episodes, seeds, final):
    ...
    postplant_wins = sum(b["episodes"] * b["plant_rate"] * b["postplant_win_rate"] for b in blocks)
    return {
        "episodes": episodes * len(blocks), "seeds": [int(s) for s in seeds],
        ...
        "registered_plant_rate": float(np.mean([b["registered_plant_rate"] for b in blocks])),
        "worst_registered_plant_rate": float(min(b["registered_plant_rate"] for b in blocks)),
        "worst_carry_no_entry_rate": float(max(b["carry_no_entry_rate"] for b in blocks)),
        "worst_timeout_rate": float(max(b["timeout_rate"] for b in blocks)),
        "worst_carrier_preentry_death_rate": float(max(b["carrier_preentry_death_rate"] for b in blocks)),
        "worst_no_entry_carrier_death_rate": float(max(b["no_entry_carrier_death_rate"] for b in blocks)),
        "guard_position_tick_rate": float(np.mean([b["guard_position_tick_rate"] for b in blocks])),
        "mean_ticks": float(np.mean([b["mean_ticks"] for b in blocks])),
        "per_seed": blocks,
        "postplant_win_rate": float(postplant_wins / max(1, plants)),
        "screened_approach_rate": sum(b["screened_approach_success_ticks"] for b in blocks) /
        ...
        "entry_formation_ready_rate": sum(b["entry_formation_ready_ticks"] for b in blocks) /
        ...
        "designated_route_block_rate": sum(b["designated_route_block_ticks"] for b in blocks) /
                                       max(1, sum(b["formation_approach_ticks"] for b in blocks)),
        "designated_route_clear_rate": sum(b["designated_route_clear_moves"] for b in blocks) /
                                       max(1, sum(b["designated_route_clear_opportunities"] for b in blocks)),
        "designated_route_clear_opportunities": sum(b["designated_route_clear_opportunities"] for b in blocks),
        "designated_route_clear_moves": sum(b["designated_route_clear_moves"] for b in blocks),
        **{key: float(np.mean([b[key] for b in blocks])) for key in
           ("carry_no_entry_rate", "carry_spawn_tick_rate", "carry_reversal_tick_rate",
            "carry_quiet_stall_tick_rate", "timeout_rate", "carrier_preentry_death_rate",
            "no_entry_carrier_death_rate", "no_entry_team_wipe_rate", "no_entry_other_rate",
            "mean_death_support_near", "mean_death_support_ahead",
            "mean_first_threat_support_near", "mean_first_threat_support_ahead",
            "death_screen_ready_rate", "first_threat_screen_ready_rate", "combat_stop_rate")},
    }
...
def train(args):
    ...
    policies = {"carry": runtime.AttackerCarryDuelingDQN(
                    obs_dim=runtime.ENTRY_SYNC_OBS_DIM, action_dim=runtime.ACTION_DIM),
                "escort": escort_runtime.DuelingQNetwork(escort_runtime.ENTRY_SUPPORT_OBS_DIM,
                                                          escort_runtime.N_ACTIONS),
                "guard": guard_runtime.AttackerGuardDuelingDQN(
                    action_dim=guard_runtime.ACTION_DIM)}
    for phase in PHASES:
        state = expand_policy_state(
            checkpoints[phase], policies[phase].feature[0].in_features,
            policies[phase].advantage_head[-1].out_features,
        )
        policies[phase].load_state_dict(state)
    import copy
    targets = {p: copy.deepcopy(net) for p, net in policies.items()}
    feature_columns = {
        "carry": range(runtime.FORMATION_OBS_DIM, runtime.ENTRY_SYNC_OBS_DIM),
        "escort": range(escort_runtime.CLEARANCE_OBS_DIM,
                        escort_runtime.ENTRY_SUPPORT_OBS_DIM),
    }
    ultimate_rows = {
        "carry": (runtime.ULTIMATE_ACTION_INDEX,),
    ...
    macro_path = Path(session.game.attacker_controller.inner_controller.macro_controller.model_path).resolve()
    macro_source = {"path": str(macro_path),
                    "sha256": hashlib.sha256(macro_path.read_bytes()).hexdigest()}
    args.output_dir.mkdir(parents=True, exist_ok=True)
    if any(args.output_dir.iterdir()):
        raise ValueError("output-dir must be empty; use a new directory for each training run")
    history, evaluations = [], []
    best = (-1, -float("inf"), -float("inf"), -1, -1, -1)
    started = time.monotonic()
    def save(kind, episode, metrics):
        bundle = {"episode": episode, "evaluation": metrics, "models": {}, "macro_source": macro_source}
        for phase in PHASES:
            payload = dict(checkpoints[phase]) if "model_state_dict" in checkpoints[phase] else {}
            payload.update(model_state_dict=policies[phase].state_dict(),
                           obs_dim=policies[phase].feature[0].in_features,
                           n_actions=policies[phase].advantage_head[-1].out_features,
                           positioning_version=VERSIONS[phase], episode=episode,
                           training_environment="actual_engine_full_round_curriculum",
                           training_revision=TRAINING_REVISION,
                           evaluation=metrics, source_models={p: str(s) for p, s in sources.items()},
                           source_hashes=hashes, macro_source=macro_source,
        ...
        (args.output_dir / (kind + "_bundle.json")).write_text(json.dumps(bundle, indent=2), encoding="utf-8")
    for episode in range(args.episodes + 1):
        global_episode = args.episode_offset + episode
        if episode:
            stats = opponent_stats(global_episode, args.curriculum_episodes, args.start_stats, args.final_stats)
            ...
            if global_episode > args.curriculum_episodes // 2 and random.random() < args.final_mix:
                stats = tuple(args.final_stats)
            # Keep exploration available when the opponent reaches full strength.
            epsilon = max(0.03, 0.20 * (1.0 - (global_episode - 1) / max(1, args.curriculum_episodes)))
            teacher_probability = (0.80 * max(0.0, 1.0 - (global_episode - 1) / args.navigation_bootstrap_episodes)
                                   if args.navigation_bootstrap_episodes else 0.0)
            row = session.play(args.seed + global_episode, stats, epsilon, teacher_probability=teacher_probability)
            row["episode"] = global_episode
            row["navigation_teacher_probability"] = teacher_probability
            row["demonstration_counts"] = {p: len(session.demonstrations[p]) for p in PHASES}
            row["navigation_demo_multiplier"] = max(teacher_probability, args.navigation_retention_weight)
            history.append(row)
            for phase in PHASES:
                if phase in args.freeze_phases:
                    continue
                transitions = session.transitions[phase]
                replays[phase].extend(n_step_transitions(transitions, args.gamma, args.n_step))
                demonstrations[phase].extend(session.demonstrations[phase])
                for _ in range(min(args.max_updates, max(1, len(transitions) // 2))):
                    optimize(policies[phase], targets[phase], optimizers[phase], replays[phase], args.gamma)
                    optimize_demonstrations(policies[phase], optimizers[phase], demonstrations[phase],
                                            args.navigation_demo_weight * max(teacher_probability, args.navigation_retention_weight))
            if global_episode % 20 == 0:
                for p in PHASES:
                    targets[p].load_state_dict(policies[p].state_dict())
            if global_episode % 10 == 0:
                recent = history[-50:]
                print(f"[EP {global_episode}] opponent={tuple(round(x, 1) for x in stats)} "
        ...
        if episode == 0 or global_episode % args.eval_interval == 0 or episode == args.episodes:
            metrics = evaluate_multi(session, args.eval_episodes, args.eval_seeds, args.final_stats)
            metrics["episode"] = global_episode
            metrics["entry_quality_passed"] = entry_quality_passed(metrics, args.max_no_entry_rate, args.max_timeout_rate)
            evaluations.append(metrics)
            score = selection_score(metrics, args.max_no_entry_rate, args.max_timeout_rate)
            ...
            (args.output_dir / "evaluation_history.json").write_text(json.dumps(evaluations, indent=2), encoding="utf-8")
    ...
    for phase in PHASES:
        payload = torch.load(args.output_dir / f"dqn_attacker_{phase}_gc_best_by_eval.pt", weights_only=False)
        policies[phase].load_state_dict(payload["model_state_dict"])
    holdout = evaluate_multi(session, args.holdout_episodes, args.holdout_seeds, args.final_stats)
    holdout["meets_win_rate_target"] = holdout["worst_round_win_rate"] >= args.target_win_rate
    holdout["entry_quality_passed"] = entry_quality_passed(holdout, args.max_no_entry_rate, args.max_timeout_rate)
    holdout["ready_for_match_validation"] = holdout["meets_win_rate_target"] and holdout["entry_quality_passed"]
    holdout["target_win_rate"] = args.target_win_rate
    holdout["selected_episode"] = json.loads((args.output_dir / "best_by_eval_bundle.json").read_text(encoding="utf-8"))["episode"]
    (args.output_dir / "holdout_evaluation.json").write_text(json.dumps(holdout, indent=2), encoding="utf-8")
...
def main():
    ...
    for phase in PHASES:
        parser.add_argument("--init-" + phase, type=Path, default=ROOT / "gc_v1" / "data" /
                            f"attacker_{phase}_gc_data" / f"dqn_attacker_{phase}_gc_best_by_eval.pt")
    ...
    parser.add_argument("--start-stats", type=float, nargs=3, default=(50, 10, 0), metavar=("HIT", "HS", "DODGE"))
    parser.add_argument("--final-stats", type=float, nargs=3, default=(100, 60, 40), metavar=("HIT", "HS", "DODGE"))
    ...
    parser.add_argument("--eval-seeds", type=int, nargs="+", default=(3026091700, 5026091700, 6026091700))
    parser.add_argument("--holdout-seeds", type=int, nargs="+", default=(9026092000, 10026092000, 11026092000))
    ...
    parser.add_argument("--navigation-bootstrap-episodes", type=int, default=500,
    ...
    parser.add_argument("--navigation-retention-weight", type=float, default=0.15,
                        help="minimum demo loss multiplier after bootstrap; labels never replace eval/inference actions")
    parser.add_argument("--n-step", type=int, default=5)
    parser.add_argument("--freeze-phases", nargs="*", choices=PHASES, default=(),
                        help="keep selected phase weights fixed while preserving their deployed behavior")
    parser.add_argument("--feature-only-phases", nargs="*", choices=("carry", "escort"), default=(),
                        help="train only coordination-feature columns; zero-feature behavior stays exact")
    parser.add_argument("--action-only-phases", nargs="*", choices=PHASES, default=(),
                        help="also train only the new ultimate advantage-output row")
    args = parser.parse_args()
    if min(args.episodes, args.eval_interval, args.eval_episodes, args.holdout_episodes, args.max_updates) < 1:
        parser.error("episode counts, evaluation interval and max-updates must be positive")
    ...
    if not 0 <= args.curriculum_episodes < total_episodes:
        parser.error("curriculum-episodes must be >= 0 and smaller than total episodes (offset + additional episodes)")
    if (args.navigation_bootstrap_episodes < 0 or args.navigation_demo_weight < 0
            or not 0 <= args.navigation_retention_weight <= 1 or args.n_step < 1):
        parser.error("navigation bootstrap counts/weights must be nonnegative and n-step positive")
    if args.navigation_bootstrap_episodes >= total_episodes:
        parser.error("navigation-bootstrap-episodes must be smaller than total episodes (reserve policy-only training)")
    if (not 0 < args.gamma <= 1 or args.lr <= 0 or not 0 <= args.target_win_rate <= 1
            or not 0 <= args.final_mix <= 1 or not 0 <= args.max_no_entry_rate <= 1
    ...
    seed_sets = [set(range(args.seed + args.episode_offset + 1,
                           args.seed + args.episode_offset + args.episodes + 1))]
    seed_sets.extend(set(range(s, s + args.eval_episodes)) for s in args.eval_seeds)
    seed_sets.extend(set(range(s, s + args.holdout_episodes)) for s in args.holdout_seeds)
    if any(seed_sets[i] & seed_sets[j] for i in range(len(seed_sets)) for j in range(i)):
