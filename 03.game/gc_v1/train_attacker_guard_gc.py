"""gc_v1/train_attacker_guard.py

固定チーム(Xdll/SyouTa/Absol/eKo/SugarZ3ro)専用の
Attacker「guard phase」学習スクリプト(プラント後、解除阻止に特化)。

train_defender_search.py(gc_v1)をベースに、以下を置き換えた:
  - このフェーズでは gc_v1 固定チームは Attacker側(A)。
  ...
  - map_data_guard_plant_gc.py の5～9で設置パターンを登録し、
    map_data_guard_gc.py の同じ数字5～9を、その設置に対応する
    Guard候補として使用する。
  - 同じ数字は複数配置可能。Guard候補が5人分より多い場合は
    エピソードごとに5人分を抽選する。

完全に自己完結。run_game.py / controllers.py / battle_logic.py /
abilities_los.py などのfeatureモジュールは一切importしない。
character_stats_gc.py / game_core.py / map_data.py は定数専用
ファイルとして参照する(import制限の対象外)。

学習データ・チェックポイントは gc_v1/data/attacker_guard_gc_data/
以下に保存する。
...
# ---------------------------------------------------------------------------
# gc_v1 固定チーム定義(train_defender_search.pyと同一)
# ---------------------------------------------------------------------------
...
print("[gc_v1] 固定チーム(Attacker/guard) 確定ステータス:")
...
# Guard専用設置パターンマップ(5～9)
_GUARD_PLANT_GRID = _parse_grid(GUARD_PLANT_MAZE_STR)
if _GUARD_PLANT_GRID.shape != GRID.shape:
    raise RuntimeError(
        f"map_data_guard_plant_gc.pyのマップサイズ{_GUARD_PLANT_GRID.shape}が"
...
PLANT_PATTERN_CELLS = {
    marker: [
        ...
        if int(_GUARD_PLANT_GRID[r, c]) == marker
    ]
    for marker in GUARD_PATTERN_MARKERS
}

# 未使用の数字は許可する。設置位置がある数字だけを有効パターンにする。
ACTIVE_GUARD_PATTERNS = [
    marker for marker in GUARD_PATTERN_MARKERS if PLANT_PATTERN_CELLS[marker]
]

if not ACTIVE_GUARD_PATTERNS:
    raise RuntimeError(
        "map_data_guard_plant_gc.py に設置パターン(5～9)が1つもありません。"
        "本来のプラント可能マス(2)の一部を5～9へ置き換えてください。"
    )

# 設置位置マーカーは本番マップでプラント可能(2)であることを必須とする。
for _marker in ACTIVE_GUARD_PATTERNS:
    for _pos in PLANT_PATTERN_CELLS[_marker]:
        if _pos not in PLANT_CELLS:
            raise RuntimeError(
                f"設置パターン{_marker}の{_pos}は、map_data.py上で"
                "プラント可能マス(2)ではありません。"
            )
...
# ============================================================================
# 設置パターン5～9と、それに対応するGuard候補5～9
# ============================================================================
_GUARD_GRID = _parse_grid(GUARD_MAZE_STR)
if _GUARD_GRID.shape != GRID.shape:
    raise RuntimeError(
        f"map_data_guard_gc.pyのマップサイズ{_GUARD_GRID.shape}が"
        f"map_data.pyの{GRID.shape}と一致しません。"
    )
...
GUARD_PATTERN_CELLS = {
    marker: [
        ...
        for r in range(HEIGHT)
        for c in range(WIDTH)
        if int(_GUARD_GRID[r, c]) == marker
    ]
    for marker in GUARD_PATTERN_MARKERS
}

for _marker in ACTIVE_GUARD_PATTERNS:
    if not GUARD_PATTERN_CELLS[_marker]:
        raise RuntimeError(
            f"設置パターン{_marker}は存在しますが、map_data_guard_gc.py側に"
            f"対応するGuard位置{_marker}が1つもありません。"
        )
    for _pos in GUARD_PATTERN_CELLS[_marker]:
        if GRID[_pos[0], _pos[1]] == 1:
            raise RuntimeError(
                f"Guardパターン{_marker}の{_pos}がmap_data.py上の壁(1)です。"
            )


def _nearest_registered_pattern_for_plant(plant_pos):
    """実際の設置位置に対応するパターンID(5～9)を返す。

    完全一致を最優先。どれにも一致しない緊急設置では、
    planted_posから各登録設置位置へのBFS歩行距離が最短のパターンを使う。
    同距離なら小さいmarkerを優先して決定的にする。
    """
    plant_pos = (int(plant_pos[0]), int(plant_pos[1]))

    for marker in ACTIVE_GUARD_PATTERNS:
        if plant_pos in PLANT_PATTERN_CELLS[marker]:
            return marker

    actual_dist_map = bfs_distance_map(plant_pos)
    best = None
    for marker in ACTIVE_GUARD_PATTERNS:
        for ref_pos in PLANT_PATTERN_CELLS[marker]:
            d = int(actual_dist_map[ref_pos[0], ref_pos[1]])
            if d < 0:
                continue
    ...
    # 到達不能等の異常時のみ、座標Chebyshev距離で最終フォールバック。
    return min(
        ACTIVE_GUARD_PATTERNS,
        key=lambda marker: min(
            max(abs(ref[0] - plant_pos[0]), abs(ref[1] - plant_pos[1]))
            for ref in PLANT_PATTERN_CELLS[marker]
        ),
    )


def _choose_guard_positions_for_pattern(marker, num_positions=N_ATTACKERS):
    """対応パターンのGuard候補から、このエピソードで使う5人分を選ぶ。

    候補が多い場合はランダム抽選して配置多様性を学習。
    候補が少ない場合は循環再利用し、spawn collision resolverで重複を解消する。
    """
    candidates = list(GUARD_PATTERN_CELLS[marker])
    if len(candidates) >= num_positions:
        return random.sample(candidates, num_positions)
    return [candidates[i % len(candidates)] for i in range(num_positions)]


print("[gc_v1] Guard設置パターン:")
for _marker in ACTIVE_GUARD_PATTERNS:
    print(
        f"  pattern={_marker}: "
        f"plant_cells={PLANT_PATTERN_CELLS[_marker]} "
        f"guard_cells={GUARD_PATTERN_CELLS[_marker]}"
    )
...
class UnitStub:
    def __init__(self, name, team, pos, role, has_spike=False):
        self.name = name
        self.team = team  # "A"(gc/guard) or "D"(敵/解除側)
        self.pos = list(pos)
        self.hp = MAX_HP
        self.max_hp = MAX_HP
        self.is_alive = True
        ...
        # Attacker(gc/guard)専用: 割り当てられたガードポジションとBFS距離マップ
...
# ============================================================================
# チーム共有メモリ(gc/Attacker視点。敵目撃情報のみ管理)
# ============================================================================
    ...
    def update(self, attackers, defenders, smoke_cells):
        ...
        if visible_enemies:
            ...
            if self.last_seen_enemy is not None:
                ...
                tracked = next(
                    (d for d in visible_enemies if d.name == tracked_name), None
                )
            if tracked is None:
...
def build_observation(
    ...
    obs[14] = min(dist_here if dist_here >= 0 else HEIGHT + WIDTH, HEIGHT + WIDTH) / (
        HEIGHT + WIDTH
    )
...
class GuardEnv:
    ...
    Attacker = gc_v1固定チーム(5人、固定ステータス・固定ロール、
    ...
    def __init__(self):
        ...
        self.site_idx = 0  # 旧ログ互換用。現在はpattern_markerを主に使う
    ...
    def _active_defuse_info(self):
        defuser = next(
            (d for d in self.defenders if d.is_alive and d.defuse_timer > 0), None
        )
        if defuser is None:
            return None
        return {
    ...
    def step(self, action_dict):
        ...
        for a in self.attackers:
            ...
            if use_ability:
                ...
                ability_overlap[a.name] = pre_tick_flash_recon_active and a.role in (
                    "FLASH",
                    "RECON",
                )
                if a.charges > 0:
                    if visible_enemies:
                        nearest = min(
                            visible_enemies,
                            key=lambda d: max(
                                abs(d.pos[0] - a.pos[0]), abs(d.pos[1] - a.pos[1])
                            ),
                        )
                        dist = max(
                            abs(nearest.pos[0] - a.pos[0]),
                            abs(nearest.pos[1] - a.pos[1]),
                        )
                        if dist <= ABILITY_RANGE:
                            ability_requests.append((a, tuple(nearest.pos)))
                    elif self.guard_memory.last_seen_enemy is not None:
                        ability_requests.append(
                            (a, self.guard_memory.last_seen_enemy["pos"])
                        )
                    else:
        ...
        for unit, target_pos in ability_requests:
            ...
            if unit.role == "SMOKE":
                ...
                for d in self.defenders:
                    if d.is_alive and has_los(target_pos, d.pos, smoke_cells):
                        d.reveal_remaining = max(
                            d.reveal_remaining, REVEAL_DURATION_TICKS
                        )
        ...
        if self.active_defuser_name is not None:
            active = next(
                (d for d in self.defenders if d.name == self.active_defuser_name), None
            )
        ...
        defuse_completed = any(
            d.is_alive and d.defuse_timer >= DEFUSE_REQUIRED_TICKS
            for d in self.defenders
        )
