...
class MacroEnv:
    ...
    def _apply_strategy_assignments(self, strategy, initial=False):
        ...
        if strategy == "A_RUSH":
            ...
            cfg = GC_MACRO_GROUP_SIZES["DEFAULT"]
            living_names = [a.name for a in living]
            carrier = self._carrier()
            # carrierはA/Bどちらかの2人組へ入れる。
            preferred = random.choice([SIDE_A, SIDE_B])
            groups = []
            groups += [SIDE_A] * int(cfg["A"])
            groups += [SIDE_MID] * int(cfg["MID"])
            groups += [SIDE_B] * int(cfg["B"])
            random.shuffle(groups)

            if carrier and carrier.name in living_names:
                if preferred in groups:
                    idx = groups.index(preferred)
                    groups[0], groups[idx] = groups[idx], groups[0]
                living_names.remove(carrier.name)
                living_names.insert(0, carrier.name)

            for name, side in zip(living_names, groups):
                phase = "INFO" if side != SIDE_MID else "FORWARD"
                self.assignment[name] = (side, phase, "DEFAULT")
    ...
    def _advance_assignment_phase_if_needed(self, a):
        ...
        if strategy in {"A_SPLIT", "B_SPLIT"}:
            ...
            # v14: A/B担当は情報がknownになるまでINFOセルを保持する。
            if phase == "INFO" and side in {SIDE_A, SIDE_B}:
                ...
                if (
                    ...
                    return
                if role == "DEFAULT" and random.random() < 0.50:
                    self.assignment[a.name] = (side, "LURK", "LURK")
                else:
                    self.assignment[a.name] = (side, "DEEP", role)
            elif phase == "FORWARD":
                if role == "DEFAULT" and random.random() < 0.35:
                    self.assignment[a.name] = (side, "LURK", "LURK")
                else:
                    self.assignment[a.name] = (side, "DEEP", role)
    ...
    def _update_information(self):
        ...
        # v4: last reliable information ages independently of live confidence.
        ...
        for side in (SIDE_A, SIDE_B, SIDE_MID):
            ...
            gain = info_cov * INFO_GAIN_IN_AREA + site_cov * INFO_GAIN_SITE
            if info_cov > 0.0:
                gain = max(gain, INFO_MIN_GAIN_WHILE_OCCUPIED)
            if gain > 0:
                self.info_conf[side] = min(
                    1.0,
                    self.info_conf[side] + gain,
                )

                # 情報がある時だけ敵人数推定を真値へ寄せる。
                true_count = float(defender_counts[side])
                alpha = min(1.0, 0.35 + self.info_conf[side] * 0.45)
                self.enemy_est[side] = (
                    self.enemy_est[side] * (1.0 - alpha)
                    + true_count * alpha
                )

                # v4: その時点の情報を記憶。以後live confidenceが落ちても
                # INFO_MEMORY_MAX_TICKSまではMacro判断へ使える。
                self._info_memory_count[side] = float(self.enemy_est[side])
                self._info_memory_conf[side] = float(self.info_conf[side])
            ...
            for depth_name, cells in (
                ...
                self.control[key] = max(0.0, self.control[key] - CONTROL_DECAY)
            ...
            # Pressure: forward/deep/siteにいる人数から計算。
            forward = self._area_coverage(_forward_control_cells(side))
            deep = self._area_coverage(_deep_control_cells(side))
            site = (
                self._area_coverage(_site_cells(side))
                if side in {SIDE_A, SIDE_B}
                else 0.0
            ...
            self.pressure[side] = min(
                1.0,
                0.35 * forward + 0.45 * deep + 0.70 * site,
            )
    ...
    def _remembered_enemy_info(self, side):
        """(known, enemy_count) を返す。live情報と直近memoryの強い方を使う。"""
        live_known = self.info_conf[side] >= INFO_MEMORY_MIN_CONF
        if live_known:
            return True, float(self.enemy_est[side])

        memory_known = (
            self._info_memory_age[side] <= INFO_MEMORY_MAX_TICKS
            and self._info_memory_conf[side] >= INFO_MEMORY_MIN_CONF
        )
        if memory_known:
            return True, float(self._info_memory_count[side])
...
def main():
    ...
    print("=" * 78)
    print("Ghost Champions - Attacker Macro DQN Training v15")
    print("=" * 78)
