...
class MacroEnv:
    ...
    def _apply_strategy_assignments(self, strategy, initial=False):
        ...
        if strategy == "A_RUSH":
            ...
            # v16: ランダムgroup分配をやめ、情報取得役を必ず明示する。
        ...
        for a in living:
            ...
            if (
                ...
                strategy == "DEFAULT"
                and role in {"A_SCOUT", "B_SCOUT"}
                and phase == "INFO"
            ):
                # v18: 最短INFOセルへ完全固定。確率選択はしない。
                if DEFAULT_SCOUT_USE_EXACT_NEAREST_INFO:
                    if role == "A_SCOUT":
                    ...
                    target = choose_weighted_candidate(
                        _info_cells(side),
                        a.pos,
                        +1.4,
                    )
    ...
    def _advance_assignment_phase_if_needed(self, a):
        ...
        if strategy in {"A_SPLIT", "B_SPLIT"}:
            ...
            if role in {"A_SCOUT", "B_SCOUT"} and phase == "INFO":
                # Scoutは情報knownまで専用mini-optionとしてINFOを保持。
                ...
                # 取得後は逆サイド情報を維持しやすいLurkかDeepへ。
                if random.random() < DEFAULT_SCOUT_TO_LURK_PROB:
                    self.assignment[a.name] = (side, "LURK", "SCOUT_LURK")
                else:
                    self.assignment[a.name] = (side, "DEEP", "SCOUT_DEEP")
    ...
    def _move_attackers_one_tick(self):
        ...
        if (
            ...
            priority = {
                self._default_a_scout_name: 0,
                self._default_b_scout_name: 1,
...
def main():
    ...
    print("=" * 78)
    print("Ghost Champions - Attacker Macro DQN Training v19 DIAGNOSTIC")
    print("=" * 78)
