...
class MacroEnv:
    ...
    def _infer_opposite_site_for_rotate(self):
        """v22: one HIGH site + Mid control -> opposite-site upper bound."""
        a_conf, a_count, _ = self._remembered_enemy_info_detail(SIDE_A)
        b_conf, b_count, _ = self._remembered_enemy_info_detail(SIDE_B)
        ...
        if not (
            mid_forward >= ROTATE_INFER_MID_FORWARD_MIN
            or mid_deep >= ROTATE_INFER_MID_DEEP_MIN
        ):
            return None

        # Mid人数はMEDIUM以上の時だけ差し引く。情報なしなら0として扱うが、
        # heavy側3人以上を必須にして逆site上限<=2だけを許可する。
        mid_seen = m_count if m_conf >= INFO_CONF_MEDIUM else 0.0
        candidates = []

        if a_conf >= ROTATE_INFER_SOURCE_CONF and a_count >= ROTATE_INFER_HEAVY_MIN:
            b_max = max(0.0, 5.0 - a_count - mid_seen)
            if b_max <= ROTATE_INFER_LIGHT_MAX:
                candidates.append(("A_TO_B", a_count, b_max, a_conf))

        if b_conf >= ROTATE_INFER_SOURCE_CONF and b_count >= ROTATE_INFER_HEAVY_MIN:
...
def main():
    ...
    print("=" * 78)
    print("Ghost Champions - Attacker Macro DQN Training v22")
    print("=" * 78)
