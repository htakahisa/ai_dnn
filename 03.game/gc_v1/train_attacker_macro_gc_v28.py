...
# v28: Rotate判断を短時間だけ保持し、DQNのRotate選択との時間差を吸収する。
ROTATE_OPPORTUNITY_MEMORY_STEPS = 4
...
class MacroEnv:
    def __init__(self):
        ...
        self._diag_infer_heavy_mid_overlap = 0
        self._diag_infer_bound_le_0 = 0
        self._diag_infer_bound_le_1 = 0
        self._diag_infer_bound_le_2 = 0
        self._diag_infer_bound_gt_2 = 0
        self._diag_rotate_completed_direct = 0
        self._diag_rotate_completed_inferred = 0
        # v26: latch Rotate decision for the lifetime of the Rotate option.
        self._rotate_intent_active = False
        self._rotate_intent_direction = None
        self._rotate_intent_source = None
        self._rotate_intent_snapshot = None

        self._diag_rotate_intent_created_direct = 0
        self._diag_rotate_intent_created_inferred = 0
        self._diag_rotate_intent_completed_direct = 0
        self._diag_rotate_intent_completed_inferred = 0
        self._diag_rotate_intent_expired = 0

        # v28: pre-intent Rotate opportunity memory
        self._rotate_opportunity_active = False
        self._rotate_opportunity_direction = None
        self._rotate_opportunity_source = None
        self._rotate_opportunity_snapshot = None
        self._rotate_opportunity_created_step = None
        self._rotate_opportunity_last_eval_step = None

        self._diag_rotate_opportunity_created_direct = 0
        self._diag_rotate_opportunity_created_inferred = 0
        self._diag_rotate_opportunity_used_direct = 0
        self._diag_rotate_opportunity_used_inferred = 0
        self._diag_rotate_opportunity_expired = 0
        self._diag_rotate_opportunity_direction_miss = 0
    ...
    def _urgent_smart_rotate_index(self):
        ...
        decision = self._refresh_rotate_opportunity_v28()
        if decision is None:
            return None
    ...
    def action_mask(self):
        ...
        rotate_decision = self._refresh_rotate_opportunity_v28()
        smart_a_to_b = (
    ...
    def _quantize_inferred_min_count_v27(self, estimate):
        """EWMA推定値を、Rotate推論用の最低実人数へ離散化する。
        ...
        2.5をHeavyとみなす設計と、逆site人数上限計算を整合させる。
        """
        return int(max(0, min(5, (float(estimate) + 0.5) // 1)))

    def _infer_opposite_site_for_rotate(self):
        """v25: same v24 inference, with overlap and bound distribution diagnostics."""
        a_conf, a_count, _ = self._remembered_enemy_info_detail(SIDE_A)
        b_conf, b_count, _ = self._remembered_enemy_info_detail(SIDE_B)
        ...
        if (a_heavy or b_heavy) and (forward_ok or deep_ok):
            self._diag_infer_heavy_mid_overlap += 1
        ...
        def record_bound(bound):
            if bound <= 0.0:
                self._diag_infer_bound_le_0 += 1
            elif bound <= 1.0:
                self._diag_infer_bound_le_1 += 1
            elif bound <= 2.0:
                self._diag_infer_bound_le_2 += 1
            else:
                self._diag_infer_bound_gt_2 += 1

        if a_heavy:
            a_min_count = float(
                self._quantize_inferred_min_count_v27(a_count)
            )
            mid_min_count = float(
                self._quantize_inferred_min_count_v27(mid_seen)
                if m_conf >= INFO_CONF_MEDIUM
                else 0
            )
            b_max = max(0.0, 5.0 - a_min_count - mid_min_count)
            record_bound(b_max)
        ...
        if b_heavy:
            b_min_count = float(
                self._quantize_inferred_min_count_v27(b_count)
            )
            mid_min_count = float(
                self._quantize_inferred_min_count_v27(mid_seen)
                if m_conf >= INFO_CONF_MEDIUM
            ...
            a_max = max(0.0, 5.0 - b_min_count - mid_min_count)
            record_bound(a_max)
    ...
    def _clear_rotate_opportunity_v28(self, expired=False):
        if expired and self._rotate_opportunity_active:
            self._diag_rotate_opportunity_expired += 1
    ...
    def _rotate_opportunity_age_v28(self):
        if (
            not self._rotate_opportunity_active
            or self._rotate_opportunity_created_step is None
        ):
            return None
        return max(
            0,
            int(self.macro_step - self._rotate_opportunity_created_step),
        )

    def _refresh_rotate_opportunity_v28(self):
        """Current DIRECT/INFERRED Rotate decisionを最大4 Macro step保持する。"""
        age = self._rotate_opportunity_age_v28()
        if age is not None and age > ROTATE_OPPORTUNITY_MEMORY_STEPS:
            self._clear_rotate_opportunity_v28(expired=True)

        # action_mask / urgent check / intent creationから同一stepに複数回
        # 呼ばれてもinference診断を重複加算しない。
        if self._rotate_opportunity_last_eval_step == self.macro_step:
            if (
                self._rotate_opportunity_active
                and self._rotate_opportunity_snapshot is not None
            ):
                return dict(self._rotate_opportunity_snapshot)
            return None

        self._rotate_opportunity_last_eval_step = int(self.macro_step)
        decision = self._rotate_decision_v22()

        if decision is not None:
            self._rotate_opportunity_active = True
            self._rotate_opportunity_direction = decision["direction"]
            self._rotate_opportunity_source = decision["source"]
            self._rotate_opportunity_snapshot = dict(decision)
            self._rotate_opportunity_created_step = int(self.macro_step)

            if decision["source"] == "DIRECT":
                self._diag_rotate_opportunity_created_direct += 1
            else:
                self._diag_rotate_opportunity_created_inferred += 1
    ...
    def _clear_rotate_intent_v26(self, expired=False):
        if expired and self._rotate_intent_active:
            self._diag_rotate_intent_expired += 1
    ...
    def _create_rotate_intent_v26(self, strategy):
        """v28: recent Rotate Opportunityをv26 Intentへ昇格する。"""
        if strategy not in {"ROTATE_A_TO_B", "ROTATE_B_TO_A"}:
            return False
        ...
        if decision["direction"] != expected:
            self._diag_rotate_opportunity_direction_miss += 1
            return False

        self._rotate_intent_active = True
        self._rotate_intent_direction = decision["direction"]
        self._rotate_intent_source = decision["source"]
        self._rotate_intent_snapshot = dict(decision)

        if decision["source"] == "DIRECT":
            self._diag_rotate_intent_created_direct += 1
            self._diag_rotate_opportunity_used_direct += 1
        else:
            self._diag_rotate_intent_created_inferred += 1
            self._diag_rotate_opportunity_used_inferred += 1

        # 一度Intentへ昇格したOpportunityは再利用しない。
        self._clear_rotate_opportunity_v28(expired=False)
        return True

    def _rotate_intent_matches_v26(self, direction):
        return (
            self._rotate_intent_active
            and self._rotate_intent_direction == direction
        )
    ...
    def _update_tactical_history(self):
        ...
        # v26: do not recompute the inference every subsequent step.
        # Once a Rotate decision is made, keep its intent until the option ends.
        intent_a_to_b = self._rotate_intent_matches_v26("A_TO_B")
        intent_b_to_a = self._rotate_intent_matches_v26("B_TO_A")
        ...
        # --------------------------------------------------------------
        if (
            self.current_strategy == "ROTATE_A_TO_B"
            and intent_a_to_b
        ):
            if not self._smart_rotate_completed:
                if self._rotate_intent_source == "INFERRED":
                    self._diag_rotate_completed_inferred += 1
                    self._diag_rotate_intent_completed_inferred += 1
                else:
                    self._diag_rotate_completed_direct += 1
                    self._diag_rotate_intent_completed_direct += 1

            self._smart_rotate_completed = True
            self._smart_rotate_direction = "A_TO_B"

        elif (
            self.current_strategy == "ROTATE_B_TO_A"
            and intent_b_to_a
        ):
            if not self._smart_rotate_completed:
                if self._rotate_intent_source == "INFERRED":
    ...
    def step(self, action_idx):
        ...
        if strategy != self.current_strategy:
            ...
            # v26: leaving a Rotate option clears the latched intent.
            if old_strategy in {"ROTATE_A_TO_B", "ROTATE_B_TO_A"}:
                self._clear_rotate_intent_v26(
                    expired=not self._smart_rotate_completed
                )
            ...
            if strategy in {"ROTATE_A_TO_B", "ROTATE_B_TO_A"}:
                self._clear_rotate_intent_v26(expired=False)
                self._create_rotate_intent_v26(strategy)
        ...
        if self.done and self._rotate_opportunity_active:
            self._clear_rotate_opportunity_v28(expired=True)

        if (
            self.done
            and self.current_strategy in {"ROTATE_A_TO_B", "ROTATE_B_TO_A"}
            and self._rotate_intent_active
            and not self._smart_rotate_completed
        ):
            self._diag_rotate_intent_expired += 1
            self._rotate_intent_active = False
...
def evaluate(model, episodes=EVAL_EPISODES, forced_curriculum_mode="FREE"):
    ...
    diag = {
        ...
        "infer_final": 0,
        "infer_heavy_mid_overlap": 0,
        "infer_bound_le_0": 0,
        "infer_bound_le_1": 0,
        "infer_bound_le_2": 0,
        "infer_bound_gt_2": 0,
        "rotate_completed_direct": 0,
        "rotate_completed_inferred": 0,
        "rotate_intent_created_direct": 0,
        "rotate_intent_created_inferred": 0,
        "rotate_intent_completed_direct": 0,
        "rotate_intent_completed_inferred": 0,
        "rotate_intent_expired": 0,
        "rotate_opportunity_created_direct": 0,
        "rotate_opportunity_created_inferred": 0,
        "rotate_opportunity_used_direct": 0,
        "rotate_opportunity_used_inferred": 0,
        "rotate_opportunity_expired": 0,
        "rotate_opportunity_direction_miss": 0,
    ...
    with torch.no_grad():
        for ep in range(episodes):
            ...
            diag["infer_heavy_mid_overlap"] += env._diag_infer_heavy_mid_overlap
            diag["infer_bound_le_0"] += env._diag_infer_bound_le_0
            diag["infer_bound_le_1"] += env._diag_infer_bound_le_1
            diag["infer_bound_le_2"] += env._diag_infer_bound_le_2
            diag["infer_bound_gt_2"] += env._diag_infer_bound_gt_2
            diag["rotate_completed_direct"] += env._diag_rotate_completed_direct
            diag["rotate_completed_inferred"] += env._diag_rotate_completed_inferred
            diag["rotate_intent_created_direct"] += env._diag_rotate_intent_created_direct
            diag["rotate_intent_created_inferred"] += env._diag_rotate_intent_created_inferred
            diag["rotate_intent_completed_direct"] += env._diag_rotate_intent_completed_direct
            diag["rotate_intent_completed_inferred"] += env._diag_rotate_intent_completed_inferred
            diag["rotate_intent_expired"] += env._diag_rotate_intent_expired
            diag["rotate_opportunity_created_direct"] += env._diag_rotate_opportunity_created_direct
            diag["rotate_opportunity_created_inferred"] += env._diag_rotate_opportunity_created_inferred
            diag["rotate_opportunity_used_direct"] += env._diag_rotate_opportunity_used_direct
            diag["rotate_opportunity_used_inferred"] += env._diag_rotate_opportunity_used_inferred
            diag["rotate_opportunity_expired"] += env._diag_rotate_opportunity_expired
            diag["rotate_opportunity_direction_miss"] += env._diag_rotate_opportunity_direction_miss
...
def main():
    ...
    print("=" * 78)
    print("Ghost Champions - Attacker Macro DQN Training v28")
    print("=" * 78)
    ...
    for episode in range(1, EPISODE_COUNT + 1):
        ...
        if episode % EVAL_INTERVAL == 0:
            ...
            print(
                ...
                f"heavy_mid_overlap={d['infer_heavy_mid_overlap']} "
                f"opp_bound={d['infer_opposite_bound_ok']} "
                f"bound<=0={d['infer_bound_le_0']} "
                f"bound<=1={d['infer_bound_le_1']} "
                f"bound<=2={d['infer_bound_le_2']} "
                f"bound>2={d['infer_bound_gt_2']} "
                f"final={d['infer_final']}"
            )
            print(
                "  [ROTATE OPPORTUNITY DIAG] "
                f"created_direct={d['rotate_opportunity_created_direct']} "
                f"created_inferred={d['rotate_opportunity_created_inferred']} "
                f"used_direct={d['rotate_opportunity_used_direct']} "
                f"used_inferred={d['rotate_opportunity_used_inferred']} "
                f"expired={d['rotate_opportunity_expired']} "
                f"direction_miss={d['rotate_opportunity_direction_miss']}"
            )
            print(
                "  [ROTATE INTENT DIAG] "
                f"created_direct={d['rotate_intent_created_direct']} "
                f"created_inferred={d['rotate_intent_created_inferred']} "
                f"completed_direct={d['rotate_intent_completed_direct']} "
                f"completed_inferred={d['rotate_intent_completed_inferred']} "
                f"expired={d['rotate_intent_expired']}"
            )
            print(
                "  [ROTATE COMPLETION SOURCE] "
                f"direct={d['rotate_completed_direct']} "
                f"inferred={d['rotate_completed_inferred']}"
            )
            print(
                "  [SCOUT MOVE DIAG] "
