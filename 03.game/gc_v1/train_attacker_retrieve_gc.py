...
学習する設計に変更した。エピソードごとにGC_ROSTER_ORDERから
1〜5人をランダムに抽選し、各キャラの実効ステータス・アビリティ種別
(character_stats_gc.py + GCステータス + タイガーパッシブ)
を適用する。
...
値を使っていたが、他のgc_v1学習ファイル(train_attacker_carry.py /
...
character_stats_gc.py / game_core.py は定数専用ファイルとして参照する
(import制限の対象外)。run_game.py / controllers.py は変更しない。

保存先: gc_v1/data/attacker_retrieve_gc_data/
...
MODEL_FINAL_PATH = os.path.join(DATA_DIR, "dqn_attacker_retrieve_gc_final.pt")
...
# HUNT(タイガー役)を含む4種。HUNTはチャージ0で初期化されるため
...
print("[gc_v1] 固定チーム(Attacker/retrieve) 確定ステータス:")
...
class RetrieveEnv:
    """gc_v1ロースターから毎エピソード1〜5人を抽選し、全員が対称に
    ...
    def reset(self):
        ...
        num_agents = random.randint(1, len(GC_ROSTER_ORDER))
        agent_names = random.sample(GC_ROSTER_ORDER, num_agents)
        ...
        self.agents = [
            AgentState(name, pos, GC_EFFECTIVE_STATS[name]["ability"])
