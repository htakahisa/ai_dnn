...
character_stats_gc.py は game_core.py / map_data.py と同様、
ロジックを含まない定数専用ファイルとして参照する(import制限の対象外)。

学習データ・チェックポイントは gc_v1/data/defender_search_gc_data/
以下に保存する。
...
if len(DEFENDER_SPAWNS) < len(GC_ROSTER_ORDER):
    raise RuntimeError(
        ...
        f"固定ロースター{len(GC_ROSTER_ORDER)}人分を配置できません。"
    )
...
class SearchEnv:
    ...
    Defender = gc_v1固定チーム(5人、固定ステータス・固定ロール・
    ...
    def _resolve_shots(self):
        ...
        for shooter, target in shot_intents:
            ...
            euclid = math.hypot(
                float(target.pos[0] - shooter.pos[0]),
                float(target.pos[1] - shooter.pos[1]),
            )
            hit_chance *= self._distance_hit_multiplier(euclid)
    ...
    def _compute_rewards(
        ...
        for d in self.defenders:
            ...
            if carrier is not None and self._prev_carrier_site_dist is not None:
                ...
                if progress > 0:
                    r += ATTACKER_PROGRESS_PENALTY * progress
                else:
...
def train(
    ...
    per_name_episode_total = {name: 0.0 for name in GC_ROSTER_ORDER}
    ...
    for episode in range(1, episodes + 1):
        ...
        if episode % 200 == 0:
            ...
            per_name_str = " / ".join(
                f"{name}({GC_EFFECTIVE_STATS[name]['ability']})="
