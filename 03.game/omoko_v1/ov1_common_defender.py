"""omoko_v1/common_defender.py

Defender側(search / retake)のtrain_*.pyで重複していた omoko_v1固定チーム
...
また train_defender_retake.py の元実装は未定義の RAW_STATS を
参照しておりNameErrorになるバグがあった。本ファイルでは
character_stats.CHARACTER_TABLE (name -> .hit_pct 等の属性を持つ
オブジェクト)を正しく参照する形に統一している。

【自己完結ルールとの関係】common_rl.py / common_attacker.py と同様、
omoko_v1配下のtrain_*.py同士でのimportは許容する。run_game.py /
...
COMBO_NAME = "ふわんだりぃず"
COMBO_MEMBERS = {}
COMBO_BONUS = {"accuracy": 0.15, "hs_rate": 0.10, "dodge_rate": 0.20, "reaction": 30.0}
...
def compute_effective_stats(stats_table, roster_order=ROSTER_ORDER):
    """character_stats.py の生値(stats_table)に、常時発動する
    ...
    for name in roster_order:
        ...
        if name in COMBO_MEMBERS:
            accuracy += COMBO_BONUS["accuracy"]
            hs_rate += COMBO_BONUS["hs_rate"]
            dodge_rate += COMBO_BONUS["dodge_rate"]
            reaction += COMBO_BONUS["reaction"]
        ...
        effective[name] = {
            ...
            "role": raw.role,
            "ability": ROLE_TO_ABILITY[raw.role],
        }
    return effective


def print_effective_stats(effective_stats, label="Defender", roster_order=ROSTER_ORDER):
