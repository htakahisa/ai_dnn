"""omoko_v1/learning_attacker_carry.py
...
# ---------------------------------------------------------------------------
DEFAULT_MODEL_PATH = "omoko_v1/data/attacker_carry_data/dqn_attacker_carry_best_by_eval.pt"
...
def _ray_openness(grid, pos, direction, max_range=12):
    ...
    ov1_train_attacker_carry.pyの_ray_opennessと同一ロジック。"""
    height, width = grid.shape
...
class Ov1LearningAttackerCarryController:
    """スパイク保持者(has_spike==True)のみDQNで操作する(アビリティ/PLANTのみ)。
    それ以外のomoko_v1メンバー(エスコート)はヒューリスティックで動かす。
    """
    ...
    def __init__(self, model_path=DEFAULT_MODEL_PATH, device="auto", greedy=True, epsilon=0.0, debug=False):
        ...
        self.waypoint_cells_by_site = {}
        for site, raw_cells in raw_waypoint_cells.items():
            # 旧checkpointの単一座標(tuple)と、新形式の座標リストを両対応する。
            if raw_cells and isinstance(raw_cells[0], (int, np.integer)):
                raw_cells = [raw_cells]
            self.waypoint_cells_by_site[str(site)] = [
    ...
    def set_game(self, game):
        ...
        self._waypoint_dist_maps = {
            site: [_bfs_distance_map(grid, cell) for cell in cells]
            for site, cells in self.waypoint_cells_by_site.items()
        }
    ...
    def decide_move(self, char, game_state):
        ...
        goal = (
            ...
            if not self._reached_waypoint else self._active_target
        )
        ...
        if self._reached_waypoint:
            ...
            dist_map = self._waypoint_dist_maps[self._active_site][self._waypoint_index]
