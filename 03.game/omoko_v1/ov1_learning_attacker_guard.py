"""omoko_v1/learning_attacker_guard.py

固定チーム(いぐるん/夢の街/ひつじさん/Tortlilyan/えんぺん)専用の
Attacker「guard phase」推論コントローラー(プラント後、解除阻止に特化)。

omoko_v1/train_attacker_guard.py で学習した Dueling DQN を読み込み、
...
とは無関係なので、本ファイルでは learning_defender_search.py の
...
DEFAULT_MODEL_PATH = (
    "omoko_v1/data/attacker_guard_data/"
    "dqn_attacker_guard_best_by_eval.pt"
)


# ---------------------------------------------------------------------------
# Dueling DQN (omoko_v1/train_attacker_guard.py と同一構造)
# ---------------------------------------------------------------------------
...
# も常に0として扱う(learning_defender_search.py と同じ簡略化方針)。
# ---------------------------------------------------------------------------
...
def _has_los(grid, p1, p2, smoke_cells=None):
    line_cells = _line_cells(p1, p2)
    for r, c in line_cells:
        if grid[r, c] == 1:
            return False
    if smoke_cells and len(line_cells) > 2:
...
# ---------------------------------------------------------------------------
# 警戒ポイント(watch points): ov1_train_attacker_guard.pyのSITE_PLANT_POS /
# GUARD_WATCH_POINTS_BY_SITEと必ず同じ内容に保つこと(自己完結ルールにより
# importできないため、座標は両ファイルに手動で複製する)。
# サイトの判定は、実際のplanted_posに最も近いSITE_PLANT_POSを選ぶことで行う。
# ---------------------------------------------------------------------------
from map_data import NEW_MAZE_STR as _BASE_MAZE_STR


def _cluster_plant_cells(cells, max_sites=2, cluster_radius=6):
    """ov1_train_attacker_guard.pyの_cluster_plant_cellsと同一ロジック。
    site_idxの割り当てを学習側と一致させるため、クラスタリング手順も揃える。"""
    if not cells:
        return []
    clusters = []
    for cell in cells:
...
def _compute_site_plant_positions():
    grid = _parse_grid(_BASE_MAZE_STR)
    plant_cells = [
        (r, c) for r in range(grid.shape[0]) for c in range(grid.shape[1]) if grid[r, c] == 2
    ]
    clusters = _cluster_plant_cells(plant_cells, max_sites=2)
    if not clusters:
        return [(grid.shape[0] // 2, grid.shape[1] // 2)]
    return [_pick_canonical_plant_cell(c) for c in clusters]


SITE_PLANT_POS = _compute_site_plant_positions()


def _nearest_visible_watch_point(grid, pos, watch_points, smoke_cells=None):
    visible = [wp for wp in watch_points if _has_los(grid, pos, wp, smoke_cells)]
    if not visible:
        return None
...
class _TeamMemory:
    ...
    def update(self, grid, my_team, chars, smoke_cells=None):
        ...
        for a in allies:
            for e in enemies:
                if not e.is_alive:
                    continue
                if _has_los(grid, tuple(a.pos), tuple(e.pos), smoke_cells) and e not in visible_enemies:
...
# 推論コントローラー
# ---------------------------------------------------------------------------
class Ov1LearningAttackerGuardController:
    """omoko_v1固定チーム専用、プラント後フェーズ(ガード)のAttacker AI。
    ...
    def __init__(self, model_path=DEFAULT_MODEL_PATH, greedy=True, verbose=False):
        ...
        try:
            ...
            print(f"[LOAD ERROR] attacker guard(omoko) model '{model_path}' の読込に失敗: {exc}")
        self.model.eval()
        ...
        self._debug_log_path = "attacker_guard_debug.log"
    ...
    def _maybe_advance_tick(self, char, grid, chars, smoke_cells=None):
    ...
    # 完全一致させること。
    def _build_observation(self, char, game_state, unit_has_spike_los, active_defuse_info, detonate_timer, smoke_cells=None):
        ...
        visible_enemies = [e for e in enemies if e.is_alive and _has_los(grid, char.pos, e.pos, smoke_cells)]
        ...
        # 常に0とする(learning_defender_search.py と同じ簡略化方針)。
        obs[13] = 0.0
        ...
        fx, fy = FACING_VECTORS[char.facing]
        obs[33] = fx
        obs[34] = fy

        # 警戒ポイント(watch point): 視認できていれば最も近い1点への方向。
        # ov1_train_attacker_guard.pyのbuild_observationと同一の埋め方。
    ...
    def decide_move(self, char, game_state):
        ...
        if self.verbose:
            print(
                ...
                f"action={action_idx} move={move_offset} ability={use_ability} facing={facing_dir}"
            )

        if not use_ability:
            return next_pos, {"facing": facing_dir}
