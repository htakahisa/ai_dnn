"""omoko_v1/learning_attacker_retrieve.py

固定チーム専用の
Attacker「retrieve phase」推論コントローラー(落下スパイク回収)。

omoko_v1/train_attacker_retrieve.py で学習した Dueling DQN を読み込み、
...
1人」をstickyに選ぶ(learning_attacker_guard.pyのチーム内選出と
...
DEFAULT_MODEL_PATH = (
    "omoko_v1/data/attacker_retrieve_data/"
    "dqn_attacker_retrieve_best_by_eval.pt"
)

VERBOSE_LOG = False

# ---------------------------------------------------------------------------
# Dueling DQN (omoko_v1/train_attacker_retrieve.py と同一構造。
...
# 壁のみを考慮する(スモークによる遮蔽は考慮しない。他のomoko_v1推論
# コントローラーと同じ簡略化方針)。
# ---------------------------------------------------------------------------
...
# 推論コントローラー
# ---------------------------------------------------------------------------
class Ov1LearningAttackerRetrieveController:
    """omoko_v1固定チーム専用、落下スパイク回収フェーズ(retrieve)のAttacker AI。
    ...
    def __init__(self, model_path=DEFAULT_MODEL_PATH, greedy=True, verbose=VERBOSE_LOG):
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
                ...
                if verbose:
                    ...
                    print(
                        f"[Ov1LearningAttackerRetrieveController] loaded: {model_path} "
                ...
                if verbose:
                    print(f"[Ov1LearningAttackerRetrieveController] loaded (raw state_dict): {model_path}")
            self.model.load_state_dict(state_dict)
        except Exception as exc:
            print(f"[LOAD ERROR] attacker retrieve(omoko) model '{model_path}' の読込に失敗: {exc}")
        self.model.eval()
        ...
        self._debug_log_path = "attacker_retrieve_debug.log"
    ...
    def decide_move(self, char, game_state):
        ...
        # spike_pos自体を予防的な狙点とする(learning_attacker_guard.py
        # の fallback方針と同様)。
        if visible_enemies:
            nearest = min(
                visible_enemies,
