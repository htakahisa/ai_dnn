"""omoko_v1/learning_defender_search.py

固定チーム(いぐるん/夢の街/ひつじさん/Tortlilyan/えんぺん)専用の
Defender「search phase」推論コントローラー(プラント前限定)。

omoko_v1/train_defender_search.py で学習した Dueling DQN を読み込み、
...
# ---------------------------------------------------------------------------
# Dueling DQN (omoko_v1/train_defender_search.py と同一構造)
# ---------------------------------------------------------------------------
...
def _forced_watch_facing(char, visible_enemies, team_memory):
    ...
    (ov1_train_defender_search.py側と同一方針。座標からの動的計算はしない)。"""
...
def _compute_fixed_assignments():
    ...
    最終担当地点) を ROSTER_ORDER のインデックス順にそのまま1:1
...
# 推論コントローラー
# ---------------------------------------------------------------------------
class Ov1LearningDefenderSearchController:
    """omoko_v1固定チーム専用、プラント前フェーズのDefender AI。
    ...
    def _ensure_defense_assignment(self, char, grid, chars):
        """omoko_v1固定チームは、学習時に一意に計算した固定割当
        (_FIXED_DEFENSE_ASSIGNMENT)をそのまま使う。スポーン位置・
        ...
        if _FIXED_DEFENSE_ASSIGNMENT:
            for teammate in teammates:
                setup_pos = _FIXED_SETUP_ASSIGNMENT.get(teammate.name)
                pos = _FIXED_DEFENSE_ASSIGNMENT.get(teammate.name)
                ...
                if dist_map[setup_pos[0], setup_pos[1]] < 0:
                    raise RuntimeError(
                        ...
                        f"map_data_search.pyの壁配置がmap_data.pyとズレていないか確認してください。"
                    )
    ...
    def _build_observation(
        ...
        # [36-43] 自身の向き(8方向)one-hot。ov1_train_defender_search.pyの
