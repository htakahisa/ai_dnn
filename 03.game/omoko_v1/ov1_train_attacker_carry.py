"""omoko_v1/train_attacker_carry.py
...
train_attacker_guard.py / train_defender_search.py と同一規約:
  ...
  - map_data.py / character_stats.py / game_core.py は定数専用
...
保存先: omoko_v1/data/attacker_carry_data/
...
# 保存先
# ---------------------------------------------------------------------------
DATA_DIR = "data/attacker_carry_data/"
os.makedirs(DATA_DIR, exist_ok=True)
MODEL_SAVE_PATH = os.path.join(DATA_DIR, "dqn_attacker_carry_best_by_eval.pt")
MODEL_LATEST_PATH = os.path.join(DATA_DIR, "dqn_attacker_carry_latest.pt")
...
# (learning_attacker_carry.py)のAI_CONTROLLED_SITE_SELECTIONで別途決定する。
...
print_effective_stats(EFFECTIVE_STATS, "Attacker/carry・簡略版")
...
# Defenderの初期配置候補は、carry用マップ自身の4ではなく、より多様な配置を
# 想定したov1_map_data_defender_simulate.pyの4から取得する(壁配置は同一マップ想定)。
DEFENDER_SIM_GRID = ov1_common_rl.parse_grid(DEFENDER_SIM_MAZE_STR)
if DEFENDER_SIM_GRID.shape != GRID.shape:
    raise RuntimeError(
        f"ov1_map_data_defender_simulate.pyのサイズがov1_map_data_carry.pyと一致しません。"
        f"defender_sim={DEFENDER_SIM_GRID.shape} carry={GRID.shape}"
    )
DEFENDER_SPAWNS = [
    (r, c) for r in range(HEIGHT) for c in range(WIDTH)
    if DEFENDER_SIM_GRID[r, c] == 4 and GRID[r, c] != 1
]
    ...
    for r in range(height):
        for c in range(width):
            if grid[r, c] == 1:
                continue
            open_dirs = [
                (dr, dc) for dr, dc in ov1_common_rl.CARDINAL_MOVES
...
for _site, _value in WAYPOINT_VALUE_BY_SITE.items():
    ...
    if _cells:
        WAYPOINT_CELLS_BY_SITE[_site] = _cells
    else:
        print(
...
WAYPOINT_DIST_MAPS = {
    site: [bfs_distance_map(cell) for cell in cells]
...
class UnitStub:
    def __init__(self, name, team, pos, role, ability, accuracy, dodge_rate, hs_rate, reaction, has_spike=False):
        self.name = name
        self.team = team  # "A"(omoko/carry+escort) or "D"(敵)
        self.pos = list(pos)
        self.hp = MAX_HP
        self.max_hp = MAX_HP
        self.is_alive = True
...
# AttackerCarryDuelingDQN(hidden=64) は ov1_common_rl.DuelingQNet(OBS_DIM, ACTION_DIM, hidden=64)
...
def decode_action(action_idx):
    ...
    両者は完全に独立(ov1_train_defender_search.pyのdecode_actionと同一規約)。
...
class CarryEnv:
    ...
    def reset(self):
        ...
        if handoff:
            carrier_name = random.choice(ROSTER_ORDER)
        else:
    ...
    def _current_nav_goal(self):
        if not self.reached_waypoint:
            return WAYPOINT_CELLS_BY_SITE[self.active_site][self.waypoint_index]
    ...
    def step(self, action_idx):
        ...
        if self.carrier.is_alive and not self.reached_waypoint:
            ...
            waypoint_cell = WAYPOINT_CELLS_BY_SITE[self.active_site][self.waypoint_index]
