"""omoko_v1/train_attacker_escort.py
...
3. キャリアー役の可変化(ハンドオフ想定)
   ...
   通常は「ひつじさん」がキャリアーだが、一定確率で他のロースター
...
保存先: omoko_v1/data/attacker_escort_data/
...
_DEFENDER_SIM_GRID = ov1_common_rl.parse_grid(DEFENDER_SIM_MAZE_STR)
...
N_ESCORTS = 4  # omoko固定チーム5人からキャリアー1人を除いた人数
...
HANDOFF_AUGMENT_PROB = 0.25  # 一定確率でキャリアー役をひつじさん以外から選ぶ(train_attacker_carry.pyと同一方針)

FACING_ALIGN_WEIGHT_ESCORT = 0.01  # 敵不可視時のみ有効。進行方向を向くほど+、背を向けるほど-(弱いshaping)

LINEUP_ABILITY_MARKERS = {"S": "SMOKE", "R": "RECON", "F": "FLASH"}  # マップ上の定点マーカー文字→アビリティ種別
LINEUP_TRIGGER_RADIUS = 6          # キャリアーがこの半径内に接近したら、射線が無くても定点使用を許可する
LINEUP_ABILITY_REWARD = 2.0        # 定点アビリティ使用のボーナス(ability_success_rewardと同水準)

POSITION_HINT_DECAY = (1.0, 0.9, 0.75)  # 警戒点の推奨立ち位置からの距離(0/1/2マス)ごとの得点倍率。3マス以上は対象外
POSITION_HINT_WEIGHT = 0.05             # 推奨立ち位置報酬の基礎係数
...
WATCH_POINT_ALIGN_WEIGHT = 0.03     # 敵情報が無い時だけ有効な警戒点facing shaping
...
print_effective_stats(EFFECTIVE_STATS, "Attacker/escort")
...
def _has_los(grid, smoke_cells, p1, p2):
    """壁とスモークを考慮した射線判定(ov1_common_rl.has_los利用。引数順序が
...
def _direction_alignment(facing, from_pos, to_pos):
    ...
    ov1_train_attacker_guard.pyの警戒ポイントshapingと同一の考え方。"""
    dc = float(to_pos[1] - from_pos[1])
    dr = float(to_pos[0] - from_pos[0])
    dist = math.hypot(dc, dr)
...
class EscortEnv:
    ...
    def decode_action(action_idx):
        ...
        (ov1_train_attacker_carry.pyのdecode_actionと同一規約)。"""
        idx = int(action_idx)
    ...
    def __init__(
        ...
        lineup_ability_reward=LINEUP_ABILITY_REWARD,
        kill_bonus=5.0,
        death_penalty=3.0,
        ...
        self.lineup_ability_reward = lineup_ability_reward
        ...
        # Defenderの初期配置候補は、escort用マップ自身の4ではなく、より多様な配置を
        # 想定したov1_map_data_defender_simulate.pyの4から取得する(壁配置は同一マップ想定)。
        if _DEFENDER_SIM_GRID.shape != self.grid.shape:
            raise RuntimeError(
                f"ov1_map_data_defender_simulate.pyのサイズがescortマップと一致しません。"
                f"defender_sim={_DEFENDER_SIM_GRID.shape} escort={self.grid.shape}"
            )
        ...
        self.escort_stuck = []
        self.escort_watch_points = [None] * self.n_escorts
    ...
    def _refresh_watch_assignments(self):
        """現在LOSが通る警戒点を、生存escortへできるだけ均等に割り当てる。"""
        alive = [i for i in range(self.n_escorts) if self.escort_alive[i]]
        if not alive or not self.watch_points:
            self.escort_watch_points = [None] * self.n_escorts
            return

        smoke_cells = self._smoke_cell_set()
        usable = [
            point for point in self.watch_points
            if any(_has_los(self.grid, smoke_cells, self.escort_pos[i], point) for i in alive)
        ]
        if not usable:
            self.escort_watch_points = [None] * self.n_escorts
            return

        assignments = [None] * self.n_escorts
        for rank, i in enumerate(alive):
            assignments[i] = usable[rank % len(usable)]
        self.escort_watch_points = assignments

    def _watch_point_info(self, i):
        point = self.escort_watch_points[i]
        if point is None:
            return 0.0, 0.0, 0.0
        r, c = self.escort_pos[i]
        if not _has_los(self.grid, self._smoke_cell_set(), (r, c), point):
            return 0.0, 0.0, 0.0
        return 1.0, (point[0] - r) / max(1, self.height), (point[1] - c) / max(1, self.width)

    def _position_hint_info(self, i):
        """割り当てられた警戒点に対応する推奨立ち位置(大文字ヒント)への方向情報。
        LOSの有無は問わない(まずそこへ移動することを促すための情報のため)。"""
        point = self.escort_watch_points[i]
        if point is None:
            return 0.0, 0.0, 0.0
        hints = self.watch_point_hints.get(point) or []
        if not hints:
            return 0.0, 0.0, 0.0
        r, c = self.escort_pos[i]
        nearest = min(hints, key=lambda cell: _chebyshev((r, c), cell))
        return 1.0, (nearest[0] - r) / max(1, self.height), (nearest[1] - c) / max(1, self.width)

    def _position_hint_reward(self, i):
        """推奨立ち位置ちょうどで最大、1〜2マスずれても緩やかな減衰で加点する。
        ぴったりその地点でなくても角度さえつけば有効、という運用に合わせた設計。"""
        point = self.escort_watch_points[i]
        if point is None:
            return 0.0
        hints = self.watch_point_hints.get(point) or []
        if not hints:
            return 0.0
        dist = min(_chebyshev(self.escort_pos[i], cell) for cell in hints)
        if dist >= len(POSITION_HINT_DECAY):
            return 0.0
        return POSITION_HINT_DECAY[dist] * POSITION_HINT_WEIGHT
    ...
    def _resolve_spawn_collision(self, pos, occupied):
        ...
        (ov1_common_rl.resolve_spawn_collision使用)。"""
        return ov1_common_rl.resolve_spawn_collision(self.grid, pos, occupied)
    ...
    def reset(self):
        ...
        if handoff:
            carrier_name = self.rng.choice(ROSTER_ORDER)
        else:
            carrier_name = SPIKE_HOLDER
        self.carry_name = carrier_name

        carrier_stats = EFFECTIVE_STATS[carrier_name]
        ...
        carry_base_pos = spawn_positions[ROSTER_ORDER.index(carrier_name) % len(spawn_positions)]
        ...
        for i, name in enumerate(escort_names):
            ...
            base_pos = spawn_positions[ROSTER_ORDER.index(name) % len(spawn_positions)]
        ...
        self._refresh_watch_assignments()
    ...
    def _available_lineup_cell(self, ability, pos):
        ...
        「キャリアーが定点からLINEUP_TRIGGER_RADIUS以内に接近している」
        ...
        candidates = []
        for cell in self.ability_lineup_points.get(ability, []):
            if cell in used:
                continue
    ...
    def get_action_mask(self, i):
        ...
        if self.escort_ability_used[i]:
            ...
            # ABILITY_RANGEで判定する。
            # 視認可能な敵がいなくても、マップ上の定点(S/R/F、
            # ability_lineup_points)のうち未使用のものが使用可能条件
            # (キャリアーが接近している、または射程内・射線内)を満たせば
            # アンマスクする。
            ...
            if not has_target:
                has_target = self._available_lineup_cell(ability_type, (r, c)) is not None
            if not has_target:
                base_mask[self.ACTION_ABILITY] = False
    ...
    def _get_obs(self, i):
        ...
        # 現在有効な警戒点のうち、自分に割り当てられた点の情報。
        # 割り当ては全escortで均等化し、敵情報がある場合は報酬側で抑制する。
        watch_visible, watch_dr, watch_dc = self._watch_point_info(i)
        obs.extend([watch_visible, watch_dr, watch_dc])

        # その警戒点に対応する推奨立ち位置(大文字ヒント)への方向。
        # ちょうどその地点でなくても2マス程度のズレは許容する運用のため、
        # LOSの有無に関わらず常に方向を提示する。
    ...
    def _apply_ability(self, i):
        ...
        if ability in ("FLASH", "RECON"):
            ...
            if enemy_idx is None:
                # 視認可能な敵がいない場合、マップ上の定点(S/R/F)への
                # 投擲をフォールバックとして許可する。ACTION_ABILITYをDQNが
                # 選んだ場合にのみここへ到達するため、無条件自動投擲と異なり
                # action選択に正しく報酬が紐づく。
                lineup_cell = self._available_lineup_cell(ability, pos)
                if lineup_cell is not None:
                    self.lineup_used_cells[ability].add(lineup_cell)
                    return self.lineup_ability_reward
        ...
        if ability == "SMOKE":
            ...
            if enemy_idx is None:
                ...
                if lineup_cell is not None:
                    ...
                    cells = {
                        (rr, cc)
                        for rr in range(lineup_cell[0] - 1, lineup_cell[0] + 2)
                        for cc in range(lineup_cell[1] - 1, lineup_cell[1] + 2)
        ...
        for shooter_kind, shooter_idx, target_kind, target_idx in shooters:
            ...
            if target_kind == "carry":
                ...
                if self.enemy_hp[target_idx] <= 0:
                    ...
                    if shooter_kind == "carry" and EFFECTIVE_STATS[self.carry_name]["ability"] == "HUNT":
                        self.carry_hp = min(MAX_HP, self.carry_hp + 50)
                    elif shooter_kind == "escort" and self.escort_ability_type[shooter_idx] == "HUNT":
                        self.escort_hp[shooter_idx] = min(MAX_HP, self.escort_hp[shooter_idx] + 50)
    ...
    def step(self, actions):
        ...
        # 選んだ方向を優先する(ov1_train_attacker_carry.pyと同一方針)。
        ...
        # ov1_train_attacker_carry.pyと同一方針: 進行方向を向くほど+、背を
        ...
        # 5.8 敵情報が無い時だけ、均等配分された警戒点を向くよう促す。
        # 警戒点は移動目標ではなく、LOSが通る場合のfacing shapingに限定する。
        if team_sighting_for_reward is None:
            for i in range(self.n_escorts):
                if not self.escort_alive[i] or pre_action_visible_enemy[i]:
                    continue
                if self.escort_facing_forced_this_tick[i]:
                    continue
                point = self.escort_watch_points[i]
                if point is None or not _has_los(
                    self.grid, self._smoke_cell_set(), self.escort_pos[i], point
                ):
                    continue
                alignment = _direction_alignment(
                    self.escort_facing[i], self.escort_pos[i], point
                )
                rewards[i] += WATCH_POINT_ALIGN_WEIGHT * alignment

        # 5.9 警戒点の推奨立ち位置(大文字ヒント)に近いほど加点する。
        # ちょうどその地点が最大、1〜2マスのズレは緩やかな減衰で許容する。
        # 5.8と同じく、敵情報が無い(探索フェーズの)時だけ有効にする。
        if team_sighting_for_reward is None:
            for i in range(self.n_escorts):
                if not self.escort_alive[i] or pre_action_visible_enemy[i]:
                    continue
                rewards[i] += self._position_hint_reward(i)
...
# ---------------------------------------------------------------------------
# Dueling DQN(重み共有。他のomoko_v1学習ファイルと同一アーキテクチャ)
# ---------------------------------------------------------------------------
Transition = namedtuple("Transition", ("state", "action", "reward", "next_state", "next_mask", "done"))
# DuelingQNetwork(hidden=128) は ov1_common_rl.DuelingQNet と層構成が完全一致。
# ReplayBuffer / select_action / optimize_model は ov1_common_rl に統合。
# 注意: 元のselect_actionはマスク全滅を想定していなかった(ACTION_STAYが
# 常にTrueのため)。ov1_common_rl.select_actionはfallback_action引数を持つが、
# デフォルト0でも実質未使用となる想定。
...
def main():
    parser = argparse.ArgumentParser(description="omoko_v1 Attacker Escort Phase 学習スクリプト")
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="data/attacker_escort_data/",
    )
    ...
    def _make_checkpoint(episode, success_rate, avg_reward, avg_block_events):
        return {
            ...
            "spike_holder_default": SPIKE_HOLDER,
            "hold_ticks": args.hold_ticks,
            "ability_lineup_points": {k: list(v) for k, v in env.ability_lineup_points.items()},
        }
