"""omoko_v1/train_attacker_guard.py

固定チーム(いぐるん/夢の街/ひつじさん/Tortlilyan/えんぺん)専用の
Attacker「guard phase」学習スクリプト(プラント後、解除阻止に特化)。

train_defender_search.py(omoko_v1)をベースに、以下を置き換えた:
  - このフェーズでは omoko_v1 固定チームは Attacker側(A)。
...
character_stats.py / game_core.py / map_data.py は定数専用
ファイルとして参照する(import制限の対象外)。

学習データ・チェックポイントは omoko_v1/data/attacker_guard_data/
以下に保存する。
...
# 保存先
# ---------------------------------------------------------------------------
DATA_DIR = "data/attacker_guard_data/"
os.makedirs(DATA_DIR, exist_ok=True)
MODEL_SAVE_PATH = os.path.join(DATA_DIR, "dqn_attacker_guard_best_by_eval.pt")
MODEL_LATEST_PATH = os.path.join(DATA_DIR, "dqn_attacker_guard_latest.pt")
...
N_ATTACKERS = 5  # omoko固定チーム(このフェーズではAttacker)
...
DEFENDER_SMOKE_PROBABILITY = 0.5   # 敵に解除前スモークを持たせる確率(学習用)
DEFENDER_SMOKE_USE_RANGE = 5       # プラント地点からこの距離以内でスモークを使用する

# ---------------------------------------------------------------------------
# omoko_v1 固定チーム定義(train_defender_search.pyと同一)
# ---------------------------------------------------------------------------
EFFECTIVE_STATS = compute_effective_stats(STATS_TABLE)
print_effective_stats(EFFECTIVE_STATS, "Attacker/guard")
...
GUARD_WATCH_ALIGN_WEIGHT = 0.10       # 警戒ポイントが視認できる時のみ有効。最も近い視認可能ポイントを
                                       ...
                                       # (旧0.03ではHOLD_POSITION_BONUSに埋もれてfacingが安定しなかったため引き上げ)
GUARD_WATCH_REPOSITION_REWARD = 0.02  # 警戒ポイントが一つも視認できない場合のみ有効。持ち場(position)
                                       # モード中、最寄りの警戒ポイントへBFS距離を縮めるほど+
MAX_SIGHTING_RESPONDERS = 2  # 目撃情報に反応して持ち場を離れてよい人数の上限(残りは持ち場を維持)
KILL_REWARD = 0.5
...
def _bfs_next_step(start, goal, occupied, allow_adjacent_goal=True):
    """敵(Defender)ヒューリスティック用。ov1_common_rl.bfs_next_step使用。"""
    return ov1_common_rl.bfs_next_step(GRID, start, goal, occupied, allow_adjacent_goal=allow_adjacent_goal)


def _random_step(pos, occupied):
    return ov1_common_rl.random_step(GRID, pos, occupied)
...
def _direction_alignment(facing, from_pos, to_pos):
    ...
    ov1_train_attacker_carry.pyのFACING_ALIGN_WEIGHT計算と同一の考え方だが、
    対象が「移動方向」ではなく固定座標(警戒ポイント)である点が異なる。"""
    dc = float(to_pos[1] - from_pos[1])
    dr = float(to_pos[0] - from_pos[0])
    dist = math.hypot(dc, dr)
...
_GUARD_GRID = ov1_common_rl.parse_grid(GUARD_MAZE_STR)
...
print("[omoko_v1] サイト別プラント地点・ガードポジション:")
...
def _multi_source_bfs_distance_map(source_cells):
    if not source_cells:
        return None
    height, width = GRID.shape
    ...
    while q:
        r, c = q.popleft()
        for dr, dc in ov1_common_rl.CARDINAL_MOVES:
            nr, nc = r + dr, c + dc
...
class UnitStub:
    def __init__(self, name, team, pos, role, has_spike=False):
        self.name = name
        self.team = team  # "A"(omoko/guard) or "D"(敵/解除側)
        self.pos = list(pos)
        self.hp = MAX_HP
        self.max_hp = MAX_HP
        self.is_alive = True
        ...
        # 向き。Attacker(guard)は北向き、Defenderは南向きでスタート(game_core.Characterと同一)。
        ...
        # Attacker(omoko/guard)専用: 割り当てられたガードポジションとBFS距離マップ
        ...
        self.prev_watch_reposition_dist = None


def _resolve_spawn_collision(pos, occupied):
    """ガードポジション候補が重複/壁だった場合に、BFSで最寄りの空きマスへ逃がす。"""
    return ov1_common_rl.resolve_spawn_collision(GRID, pos, occupied)

def _build_fixed_attackers(site_idx, guard_positions):
    """omoko_v1固定チーム(5人)をAttacker(guard側)として生成する。
    ロースター順にガードポジションを1人ずつ割り当てる(重複時は隣接空きマスへ逃がす)。
    ステータス・ロールは_compute_effective_statsで確定済みの値をそのまま使う。"""
    attackers = []
    occupied = set()
...
def _build_defenders():
    ...
    ランダムスポーンさせる。一定確率で1人だけSMOKEを持たせ、プラント地点
    付近で一度だけ使用させることで、Attacker(guard)側がスモークで
    LOSが通らない状況でもスパイクへ詰める行動を学習できるようにする。
    将来、敵専用の固定チームに差し替える場合はこの関数だけを変更すればよい。"""
    d_spawns = random.sample(DEFENDER_SPAWNS, min(N_DEFENDERS, len(DEFENDER_SPAWNS)))
    smoker_idx = (
        random.randrange(len(d_spawns))
        if random.random() < DEFENDER_SMOKE_PROBABILITY
        else -1
    )
    return [
        UnitStub(f"D{i+1}", "D", pos, "SMOKE" if i == smoker_idx else "NONE")
        for i, pos in enumerate(d_spawns)
    ]


# ============================================================================
# チーム共有メモリ(omoko/Attacker視点。敵目撃情報のみ管理)
# ============================================================================
...
# AttackerGuardDuelingDQN(hidden=128) は ov1_common_rl.DuelingQNet と層構成が完全一致。
# ReplayBufferは共通版を使用。
...
def build_observation(
    ...
    fx, fy = FACING_VECTORS[unit.facing]
    obs[33] = fx
    obs[34] = fy

    # 警戒ポイント(watch_point_info): 視認できる警戒ポイントのうち最も近いもの
    # への方向。visibleフラグが立っている間だけ、対応するfacingを選ぶ学習を
    # shaping報酬(GUARD_WATCH_ALIGN_WEIGHT)で後押しする。
    if watch_point_info is not None:
        obs[35] = 1.0
...
def decode_action(action_idx):
    ...
    base_idxはmove_idx(0-4)*2+use_ability_flag(0/1)、facing_idxは向き
...
def build_action_mask(unit, occupied, lock_movement=False):
    ...
    base(0-9)側のマスクをfacing方向数だけ展開する
...
class GuardEnv:
    ...
    Attacker = omoko_v1固定チーム(5人、固定ステータス・固定ロール、
    ...
    def reset(self):
        ...
        for a in self.attackers:
            ...
            a.prev_watch_reposition_dist = None
    ...
    def step(self, action_dict):
        ...
        for d in self.defenders:
            ...
            if (
                d.role == "SMOKE"
                and d.charges > 0
                and dist_to_spike <= DEFENDER_SMOKE_USE_RANGE
                and has_los(tuple(d.pos), self.planted_pos, smoke_cells)
            ):
                d.charges -= 1
                tr, tc = self.planted_pos
                cells = {
                    (rr, cc)
                ...
                self.smokes.append({
                    "cells": cells, "remaining_ticks": SMOKE_DURATION_TICKS, "team": "D",
                })
                move_plans.append((d, (0, 0)))
                continue
        ...
        # --- 味方(omoko Attacker)側: DQNの行動を反映 ---
        attacker_facing = {}
        nearest_watch_by_name = {}
        watch_alignment_info = {}
        for a in self.attackers:
        ...
        for unit, (dr, dc) in move_plans:
            ...
            # 前Tickで被弾していれば強制facingが最優先(battle_logicと同一)。
            # Attackerは明示facing(DQN選択)、Defenderは移動方向から自動計算する。
            if not unit.facing_forced_this_tick:
                explicit_facing = attacker_facing.get(unit.name)
        ...
        self.guard_memory.update(self.attackers, self.defenders, self._smoke_cells())
        self._update_sighting_dist_map()
        self._current_responders = self._compute_sighting_responders()
    ...
    def _compute_sighting_responders(self):
        """目撃情報に反応してよいのは、目撃地点にBFS距離が近い順で
        MAX_SIGHTING_RESPONDERS人まで。それ以外は持ち場(position)を維持させ、
        サイトが空になるのを防ぐ。"""
        if self.guard_memory.last_seen_enemy is None or self.sighting_dist_map is None:
            return set()
        alive_attackers = [a for a in self.attackers if a.is_alive]

        def _dist(a):
            d = self.sighting_dist_map[int(a.pos[0]), int(a.pos[1])]
    ...
    def _compute_rewards(
        ...
        for a in self.attackers:
            ...
            # 警戒ポイント(watch point): 視認できていればそこへfacingを向ける
            # ほど加点。一つも視認できていない場合、持ち場(position)モード中
            # だけ最寄りの警戒ポイントへ近づく方向へ弱く加点する(ご要望の
            # 「見えなければ移動」に相当)。defuse_alert/sightingモード中は
            # そちらの移動が最優先のため、再配置pullは行わない。
            nearest_watch = nearest_watch_by_name.get(a.name)
            if nearest_watch is not None:
                a.prev_watch_reposition_dist = None
                align_info = watch_alignment_info.get(a.name)
                if align_info is not None:
                    align_facing, align_from, align_to = align_info
                ...
                if wdist is not None and wdist < 0:
                    wdist = None
                if wdist is None or a.prev_watch_reposition_dist is None:
                    a.prev_watch_reposition_dist = wdist
                else:
                    r += GUARD_WATCH_REPOSITION_REWARD * (a.prev_watch_reposition_dist - wdist)
                    a.prev_watch_reposition_dist = wdist
            else:
                a.prev_watch_reposition_dist = None
...
# ov1_common_rl.optimize_double_dqn_step を使用(呼び出し側train()を参照)。
