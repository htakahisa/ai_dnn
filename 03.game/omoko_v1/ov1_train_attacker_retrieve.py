"""omoko_v1/train_attacker_retrieve.py
...
学習する設計に変更した。エピソードごとにROSTER_ORDERから
1〜5人をランダムに抽選し、各キャラの実効ステータス・アビリティ種別
(character_stats.py + コンボ「ふわんだりぃず」+ タイガーパッシブ)
を適用する。
...
値を使っていたが、他のomoko_v1学習ファイル(train_attacker_carry.py /
...
character_stats.py / game_core.py は定数専用ファイルとして参照する
(import制限の対象外)。run_game.py / controllers.py は変更しない。

保存先: omoko_v1/data/attacker_retrieve_data/
...
# ---------------------------------------------------------------------------
import os
DATA_DIR = "data/attacker_retrieve_data/"
os.makedirs(DATA_DIR, exist_ok=True)
MODEL_SAVE_PATH = os.path.join(DATA_DIR, "dqn_attacker_retrieve_best_by_eval.pt")
MODEL_FINAL_PATH = os.path.join(DATA_DIR, "dqn_attacker_retrieve_final.pt")
...
# 敵スタブ(学習用の簡易ディフェンダー)。他のomoko_v1学習ファイルと
...
print_effective_stats(EFFECTIVE_STATS, "Attacker/retrieve")
...
def has_los(p1, p2):
    return ov1_common_rl.has_los(GRID, p1, p2)
...
class RetrieveEnv:
    """omoko_v1ロースターから毎エピソード1〜5人を抽選し、全員が対称に
    ...
    def reset(self):
        ...
        num_agents = random.randint(1, len(ROSTER_ORDER))
        agent_names = random.sample(ROSTER_ORDER, num_agents)
        ...
        self.agents = [
            AgentState(name, pos, EFFECTIVE_STATS[name]["ability"])
...
# Dueling DQN
# ---------------------------------------------------------------------------
# DuelingQNet(hidden=128) は ov1_common_rl.DuelingQNet と層構成が完全一致。
