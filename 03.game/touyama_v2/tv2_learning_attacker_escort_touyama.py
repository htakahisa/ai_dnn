"""touyama_v2/learning_attacker_escort_touyama.py

固定チーム(Tortlilyan/いぐるん/ろびぃな/夢の街/えんぺん)専用の
Attacker Carry Phase「escort(護衛)」推論コントローラー。

train_attacker_escort.py(touyama_v2版)で学習した、Escort役4体用の
...
【観測ベクトルはtrain_attacker_escort.py(touyama_v2版)のEscortEnv._get_obs()と
...
def decode_action(action_idx):
    ...
    train_attacker_escort.py(touyama_v2版) EscortEnv.decode_action() と
...
OBS_DIM = 56  # train_attacker_escort.py(touyama_v2版) EscortEnv._obs_dim() と一致
...
class LearningAttackerEscortTouyamaController:
    """Escort Phase(touyama_v2固定チーム版)の学習済みモデルで、護衛
    ...
    # ------------------------------------------------------------------
    # 観測構築(train_attacker_escort.py(touyama_v2版)の
