"""touyama_v2/learning_attacker_guard_touyama.py

固定チーム(いぐるん/夢の街/ろびぃな/Tortlilyan/えんぺん)専用の
Attacker「guard phase」推論コントローラー(プラント後、解除阻止に特化)。

touyama_v2/train_attacker_guard.py で学習した Dueling DQN を読み込み、
...
from tv2_train_attacker_guard import GUARD_WATCH_POINTS_BY_NAME
...
DEFAULT_MODEL_PATH = (
    "touyama_v2/data/attacker_guard_touyama_data/"
    "dqn_attacker_guard_touyama_best_by_eval.pt"
)


# ---------------------------------------------------------------------------
# Dueling DQN (touyama_v2/train_attacker_guard.py と同一構造)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 警戒ポイント(watch points): tv2_train_attacker_guard.pyのSITE_PLANT_POS /
...
def _cluster_plant_cells(cells, max_sites=2, cluster_radius=6):
    """tv2_train_attacker_guard.pyの_cluster_plant_cellsと同一ロジック。
...
class LearningAttackerGuardTouyamaController:
    """touyama_v2固定チーム専用、プラント後フェーズ(ガード)のAttacker AI。
    ...
    def _build_observation(self, char, game_state, unit_has_spike_los, active_defuse_info, detonate_timer, smoke_cells=None):
        ...
        # tv2_train_attacker_guard.pyのbuild_observationと同一の埋め方。
