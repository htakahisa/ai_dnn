"""touyama_v2/learning_defender_search_touyama.py

固定チーム(いぐるん/夢の街/ろびぃな/Tortlilyan/えんぺん)専用の
Defender「search phase」推論コントローラー(プラント前限定)。

touyama_v2/train_defender_search.py で学習した Dueling DQN を読み込み、
...
touyama_v2/train_defender_search.py と同じ優先順位ツリーを踏襲する:
...
# tv2_train_defender_search.pyと完全に一致させること。
BASE_ACTION_DIM = 10
FACING_DIRS = ["N", "NE", "E", "SE", "S", "SW", "W", "NW"]
ACTION_DIM = BASE_ACTION_DIM * len(FACING_DIRS)  # 80

# 観測エンコード用。tv2_train_defender_search.pyのALL_FACINGSと同一の並び
...
# 歩行不可として扱う専用グリッド。train_defender_search.py の
# SETUP_WALK_GRID と同一ロジック(自己完結ルールによりここで複製)。
# ---------------------------------------------------------------------------
...
if _SETUP_MASK_GRID.shape != _BASE_GRID_FOR_SETUP.shape:
    raise RuntimeError(
        ...
        f"setup={_SETUP_MASK_GRID.shape} base={_BASE_GRID_FOR_SETUP.shape}"
    )
_SETUP_WALK_GRID = np.where(
    (_BASE_GRID_FOR_SETUP == 1) | (_SETUP_MASK_GRID == 1), 1, 0
).astype(np.int32)

DEFAULT_MODEL_PATH = (
    "touyama_v2/data/defender_search_touyama_data/"
...
def _bfs_best_direction_unoccupied(dist_map, grid, r0, c0, occupied):
    ...
    移動先候補から除外する。train_defender_search.py の
    bfs_best_direction_unoccupied と同一ロジック(自己完結ルールにより複製)。"""
    if dist_map is None:
        return 0, 0
    height, width = grid.shape
    cur = dist_map[r0, c0]
...
def _decode_action(action_idx):
    ...
    train_defender_search.py の decode_action と同一ロジック(自己完結ルールにより複製)。
...
def _find_marker_position(maze_str, char):
    ...
    グリッドではなく文字列を直接走査する。文字が1マスのみである前提
...
def _compute_touyama_fixed_assignments():
    """train_defender_search.py と完全に同一のロジックで固定割当を再現する。

    マップ上の文字 a,b,c,d,e (Setup Phase到達地点) / A,B,C,D,E (Search Phase
    最終担当地点) を TOUYAMA_ROSTER_ORDER のインデックス順にそのまま1:1
    対応させる(a/A=roster[0], b/B=roster[1], ...)。総当たり最適化は行わない
    (学習側で廃止されたのに合わせる)。
...
class LearningDefenderSearchTouyamaController:
    """touyama_v2固定チーム専用、プラント前フェーズのDefender AI。
    ...
    def _ensure_defense_assignment(self, char, grid, chars):
        """touyama_v2固定チームは、学習時に一意に計算した固定割当
        ...
        if _TOUYAMA_FIXED_DEFENSE_ASSIGNMENT:
            for teammate in teammates:
                setup_pos = _TOUYAMA_FIXED_SETUP_ASSIGNMENT.get(teammate.name)
                pos = _TOUYAMA_FIXED_DEFENSE_ASSIGNMENT.get(teammate.name)
                if pos is None or setup_pos is None:
                    continue
                self._assigned_positions[teammate.name] = pos
                dist_map = _bfs_distance_map(grid, pos)
                setup_dist_map = _bfs_distance_map(_SETUP_WALK_GRID, setup_pos)
                r0, c0 = int(teammate.pos[0]), int(teammate.pos[1])
                if setup_dist_map[r0, c0] < 0:
                    raise RuntimeError(
                        f"[整合性エラー] {teammate.name}: setup担当ポジション{setup_pos}へSetup Phase中の"
                        f"グリッドで到達不能です。spawn={tuple(teammate.pos)}。"
                        f"map_data_defender_setup.pyの進入禁止マスクが経路を塞いでいないか確認してください。"
                    )
                if dist_map[setup_pos[0], setup_pos[1]] < 0:
                    raise RuntimeError(
                        f"[整合性エラー] {teammate.name}: 担当ポジション{pos}へ通常グリッドで"
                        f"到達不能です。setup後地点={setup_pos}。"
                        f"map_data_search_touyama.pyの壁配置がmap_data.pyとズレていないか確認してください。"
                    )
                self._assigned_dist_maps[teammate.name] = dist_map
                self._assigned_setup_dist_maps[teammate.name] = setup_dist_map
                self._prev_defense_bfs_dist[teammate.name] = float(dist_map[r0, c0])
        ...
        for teammate in sorted(teammates, key=lambda c: c.name):
            ...
            dist_map = _bfs_distance_map(grid, pos)
            self._assigned_dist_maps[teammate.name] = dist_map
            self._assigned_setup_dist_maps[teammate.name] = _bfs_distance_map(
                _SETUP_WALK_GRID, pos
            )
    ...
    def _build_observation(
        ...
        if char_facing in ALL_FACINGS:
            obs[36 + ALL_FACINGS.index(char_facing)] = 1.0

        return obs, visible_enemies

    # -- 行動マスク ---------------------------------------------------------
    def _action_mask(self, char, grid, chars, lock_movement=False, in_setup_phase=False):
        ...
        (train_defender_search.py の build_action_mask と同一ロジック)。"""
        ...
        for move_idx, (dr, dc) in enumerate(MOVES):
            ...
            if walkable and in_setup_phase:
                walkable = _SETUP_MASK_GRID[nr, nc] == 0
            if not walkable:
                base_mask[move_idx * 2] = False
                base_mask[move_idx * 2 + 1] = False

        if in_setup_phase or _ability_charge(char) <= 0 or char.ability_name == "HUNT":
    ...
    def decide_move(self, char, game_state):
        ...
        # defender_setup_active=True 付きで呼ばれ、戻り値は座標部分だけが
        # 使われる(アビリティは実行されない)。学習側(train_defender_search.py
        # の_step_setup_phase)と同様、チーム共有メモリの更新やアビリティ
        # 使用は行わず、Setup専用マスクで移動可否のみ制限したRLの行動選択を
        # そのまま使う(BFS強制上書きはしない)。
        if game_state.get("defender_setup_active", False):
            ...
            # 移動方向はBFS最短(占有マス回避付き)で強制する。学習時のバッファも
            # この上書き後の行動で作られているため、推論側もこれに合わせる。
            # 向き(facing)は移動方向と無関係にネットワークの選択をそのまま使う。
            dist_map = self._assigned_setup_dist_maps.get(char.name)
            if dist_map is not None:
                r0, c0 = int(char.pos[0]), int(char.pos[1])
                ...
                if cur_dist > REACH_RADIUS:
                    ...
                    move_offset = _bfs_best_direction_unoccupied(
                        dist_map, grid, r0, c0, occupied_now
                    )
                else:
                    move_offset = (0, 0)
            ...
            if self.verbose:
                print(
                    f"[DEFENDER SEARCH TOUYAMA][SETUP] {char.name} "
                    f"pos={tuple(char.pos)} move={move_offset} facing={facing}"
                )
        ...
        if self.verbose:
            print(
                ...
                f"action={action_idx} move={move_offset} ability={use_ability} facing={facing}"
            )

        next_pos = [char.pos[0] + move_offset[0], char.pos[1] + move_offset[1]]

        if not use_ability:
            # 移動先(現在地の場合を含む)と向きを常にセットで返す。
            # battle_logic.py側は移動を通常通り処理しつつ、facing指定があれば
            # 移動方向からの自動計算を上書きしてこちらを優先する。
            return next_pos, {"facing": facing}

        # アビリティ使用: 狙点は「射線内の最近接の敵」を最優先、
        # いなければチーム共有メモリの直近目撃座標を使う。
        # アビリティ実行時はfacing指定を含められない(ABILITY払い出し形式のため)。
        target_pos = None
        if visible_enemies:
            nearest = min(
                visible_enemies,
