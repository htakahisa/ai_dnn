"""touyama_v2/train_attacker_guard.py

固定チーム(いぐるん/夢の街/ろびぃな/Tortlilyan/えんぺん)専用の
Attacker「guard phase」学習スクリプト(プラント後、解除阻止に特化)。

train_defender_search.py(touyama_v2)をベースに、以下を置き換えた:
  - このフェーズでは touyama_v2 固定チームは Attacker側(A)。
...
学習データ・チェックポイントは touyama_v2/data/attacker_guard_touyama_data/
以下に保存する。
...
from tv2_map_data_guard import NEW_MAZE_STR as GUARD_MAZE_STR
...
# ---------------------------------------------------------------------------
# touyama_v2 固定チーム定義(train_defender_search.pyと同一)
# ---------------------------------------------------------------------------
...
# 手動指定する(tv2_train_attacker_carry.pyのSMOKE_LINEUP_CELLS_BY_SITEと同じ
...
def _bfs_next_step(start, goal, occupied, allow_adjacent_goal=True):
    """敵(Defender)ヒューリスティック用。tv2_common_rl.bfs_next_step使用。"""
...
def _direction_alignment(facing, from_pos, to_pos):
    ...
    tv2_train_attacker_carry.pyのFACING_ALIGN_WEIGHT計算と同一の考え方だが、
...
_GUARD_GRID = tv2_common_rl.parse_grid(GUARD_MAZE_STR)
...
print("[touyama_v2] サイト別プラント地点・ガードポジション:")
...
def _build_fixed_attackers(site_idx, guard_positions):
    """touyama_v2固定チーム(5人)をAttacker(guard側)として生成する。
...
# AttackerGuardDuelingDQN(hidden=128) は tv2_common_rl.DuelingQNet と層構成が完全一致。
# ReplayBufferは共通版を使用。
...
class GuardEnv:
    ...
    Attacker = touyama_v2固定チーム(5人、固定ステータス・固定ロール、
