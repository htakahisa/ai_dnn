...
class FixedEscortController:
    ...
    def _gradient_walk(self, start, dist_map, grid, steps, seek_smaller):
    ...
    def compute_target(self, escort_pos, dist_map, grid, role, steps_override=None):
        ...
        return self._gradient_walk(escort_pos, dist_map, grid, steps, seek_smaller)
        ...
        if self.phase == "retrieve":
            ...
            if action == 4:
                if (pr, pc) == self.goal_pos:
                    ...
                    self.carry_end_reason = "planted"           # 💡追加
                    self.carry_arrival_step = self.current_step  # 💡追加
                else:
                    reward = -1.0
            else:
