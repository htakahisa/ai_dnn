...
"""AI Ver3 Phase2.7: 1vs2・連携ディフェンダー対応の設置／戦闘学習。
...
実行例:
    ...
    python train_attacker_ai_v3_phase2.py --episodes 6000 --phase1-model attacker_ai_v3_data/dqn_attacker_ai_v3_phase2_7_final.pt
...
MODEL_DIR = BASE_DIR / "attacker_ai_v3_phase2_7_data"
LATEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase2_7_latest.pt"
BEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase2_7_best.pt"
FINAL_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase2_7_final.pt"
...
class PlayerRoster:
    ...
    def enemies_of(self, player: Player) -> List[Player]:
        return [
            p for p in self.players
            if p.alive and p.team != player.team
        ]
    ...
    def occupied_positions(
        ...
        return {
            ...
            if p.alive and p is not exclude
        }
...
class Phase2OneVsTwoEnv:
    ...
    def __init__(self, seed: int = DEFAULT_SEED):
        ...
        self.defender_last_seen = {
            d.player_id: None for d in self.defenders
        }
    ...
    def random_reachable_spawn(
        ...
        candidates = [
            ...
            for pos in pool
            if pos not in avoid
            and np.isfinite(self.distance(pos, self.target_plant))
        ]
        if not candidates:
            candidates = [
                pos for pos in self.walk_cells if pos not in avoid
            ]
        if not candidates:
    ...
    def reset(
        ...
        if force_full_map or episode >= 800:
            ...
            finite = [
                pos for pos in self.walk_cells
                if np.isfinite(distance_map[pos])
                and 5 <= distance_map[pos] <= 16
            ]
    ...
    def visible_defenders(self) -> List[Player]:
        visible = [
            ...
            if self.player_visible(self.attacker, d)
        ]
    ...
    def enemy_observation_block(
        ...
        return [
            ...
            defender.hp / MAX_HP,
            min(
                chebyshev(self.attacker.pos, enemy_pos)
                / max(self.h, self.w),
                1.0,
            ),
            float(visible),
    ...
    def observation(self) -> np.ndarray:
        r, c = self.attacker.pos
        visible_ids = {
            d.player_id for d in self.visible_defenders()
        }
        ...
        out += [
            ...
            min(self.defuse_timer / DEFUSE_REQUIRED_TICKS, 1.0),
            (
                (defuser.pos[0] - r) / max(self.h - 1, 1)
                if defuser is not None else 0.0
            ),
            (
                (defuser.pos[1] - c) / max(self.w - 1, 1)
                if defuser is not None else 0.0
            ),
        ]
    ...
    def best_move_toward(
        ...
        best_actions = [
            action for value, action in choices
    ...
    def get_defuser(self) -> Optional[Player]:
        ...
        return next(
            (
                d for d in self.defenders
                if d.player_id == self.defuser_id and d.alive
            ),
            None,
        )
    ...
    def combat(self, add_reward) -> None:
        ...
        for shooter in order:
            ...
            if (
                shooter.team == Team.DEFENDER
                and not self.defender_should_shoot(shooter)
            ):
                continue
    ...
    def step(self, action: int):
        ...
        if (
            self.attacker.pos in self.plant_cells
