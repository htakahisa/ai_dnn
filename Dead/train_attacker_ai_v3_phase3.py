...
"""AI Ver3 Phase3.0: 戦術観測追加・Phase2.7部分転移対応の1vs2学習。
...
MODEL_DIR = BASE_DIR / "attacker_ai_v3_phase3_data"
LATEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase3_latest.pt"
BEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase3_best.pt"
FINAL_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v3_phase3_final.pt"
...
class Phase3OneVsTwoEnv:
    ...
    def defender_should_shoot(self, defender: Player) -> bool:
        ...
        cover_has_los = any(
            ...
            and has_los(teammate.pos, self.attacker.pos, self.grid)
...
class DuelingQNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        hidden = self.feature[0](x)
        extra = x[..., PHASE2_OBS_DIM:]
        hidden = hidden + self.extra_adapter(extra)
...
def adapter_debug_metrics(
    ...
    with torch.no_grad():
        ...
        extra = x[..., PHASE2_OBS_DIM:]
        adapter_hidden = model.extra_adapter(extra)
...
def train(
    ...
    CONFIG_PATH.write_text(
        json.dumps(
            {
                "phase": "phase3_0_strategic_observation",
                "obs_dim": OBS_DIM,
    ...
    if resume is not None and phase2_model is not None:
        raise ValueError("--resumeと--phase2-modelは同時に指定できません")
