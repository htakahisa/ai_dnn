...
MODEL_DIR = BASE_DIR / "attacker_ai_v4_smoke_data"
LATEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v4_smoke_latest.pt"
BEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v4_smoke_best.pt"
FINAL_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v4_smoke_final.pt"
...
R_SMOKE_USE = -0.05
R_SMOKE_INVALID = -0.8
R_SMOKE_BLOCK_IMMEDIATE = 4.0
R_SMOKE_POINTLESS = -0.15
...
class Phase3OneVsTwoEnv:
    ...
    def valid_actions(self) -> Tuple[int, ...]:
        ...
        if self.attacker.alive and self.smoke_charges > 0:
            base.extend(range(ACTION_ABILITY_UP_NEAR, ACTION_ABILITY_RIGHT_FAR + 1))
        return tuple(base)
    ...
    def handle_smoke(self, action: int, add_reward) -> None:
        ...
        visible_before = sum(
            int(has_los(self.attacker.pos, defender.pos, self.grid, set()))
        ...
        visible_after = sum(
            int(self.has_los(self.attacker.pos, defender.pos))
            for defender in self.roster.alive(Team.DEFENDER)
        )
        blocked = max(visible_before - visible_after, 0)
        if blocked > 0:
            add_reward("SMOKE_BLOCK_IMMEDIATE", R_SMOKE_BLOCK_IMMEDIATE * blocked)
        else:
...
def freeze_phase2_base_for_adapter(model: DuelingQNetwork) -> None:
    """転移済みPhase2本体を完全固定し、Adapterだけ学習可能にする。"""
    for parameter in model.parameters():
        parameter.requires_grad = False

    for parameter in model.extra_adapter.parameters():
