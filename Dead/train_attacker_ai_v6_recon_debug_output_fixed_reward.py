                ...
                q_trace.append(
                    {
                        ...
                        "recon_debug": env.recon_debug_snapshot(),
                    }
                )
...
def print_eval_q_trace(q_trace: List[Dict[str, object]]) -> None:
    ...
    for item in q_trace:
        ...
        print("[RECON-DEBUG-STEP]")
        print(
            "  ACTION                    Q      CENTER  NEW KNOWN  KNOWN% "
            "PRED ACTUAL AVAILABLE"
        )
        q_values = item.get("q_values", {})
        chosen_action = int(item["chosen_action"])
        for row in item.get("recon_debug", []):
            action = int(row["action"])
            marker = " <- chosen" if action == chosen_action else ""
            q_value = float(q_values.get(ACTION_NAMES[action], float("nan")))
            print(
                f"  {str(row['action_name']):22s} "
                f"{q_value:8.3f} "
                f"{str(row['center']):>10s} "
                f"{int(row['new_cells']):4d} "
                f"{int(row['known_cells']):5d} "
                f"{float(row['known_ratio']):7.1%} "
                f"{int(row['predicted_enemy_count']):4d} "
                f"{int(row['actual_enemy_count']):6d} "
                f"{str(bool(row['available'])):>9s}"
                f"{marker}"
            )
