...
def build_training_maps(base_text: str) -> List[Tuple[str, np.ndarray]]:
    ...
    for name, edits in layouts:
        ...
        important = list(marker_positions[2] | marker_positions[3] | marker_positions[4])
        if not important:
        ...
        if disconnected:
            raise ValueError(f"{name}: 到達不能なサイト/スポーンがあります: {disconnected}")
...
class Phase3OneVsTwoEnv:
    ...
    def recon_debug_metrics(self, action: int) -> Dict[str, object]:
        ...
        predicted_targets = self.predicted_recon_target_count(action) if available else 0
    ...
    def handle_recon(self, action: int, add_reward) -> None:
        ...
        # 新しい情報がほとんど得られなかったReconは重複使用とみなしペナルティ
        if newly_seen <= 2:
            add_reward("RECON_REPEAT", -2.0)
        else:
            add_reward(
                "RECON_NEW_CELL",
    ...
    for episode in range(1, episodes + 1):
        ...
        while not done:
            ...
            if (
                ...
                move_q_values = [
                    (move_action, float(q_values.get(ACTION_NAMES[move_action], float("-inf"))))
                    for move_action in (ACTION_UP, ACTION_DOWN, ACTION_LEFT, ACTION_RIGHT, ACTION_WAIT)
                    if move_action in env.valid_actions()
                ]
                best_move_action, best_move_q = max(move_q_values, key=lambda pair: pair[1])
                recon_debug_before_step.update(
                    {
                        ...
                        "ability_q": float(q_values.get(ACTION_NAMES[action], float("nan"))),
            ...
            if episode == 1:
                ...
                for event in pending_recon_debug:
                    event["plant_started"] = bool(
                        event.get("plant_started", False) or env.planting or env.bomb_planted
                    )
...
def print_recon_use_trace(recon_use_trace: List[Dict[str, object]]) -> None:
    ...
    for index, event in enumerate(recon_use_trace, start=1):
        ...
        print(
            f"  round_ended_early: "
            f"{bool(event['round_ended_before_5ticks'])}"
        )
    print("=" * 64)
