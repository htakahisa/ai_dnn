"""AI Ver6 Recon Multi-Map: エピソードごとにマップを切り替えてRecon行動を学習する1vs2環境。
...
- 元マップ＋左右反転＋上下反転＋180度回転をエピソードごとにランダム選択
...
実行例:
    ...
    python train_attacker_ai_v6_recon_debug_output.py --flash-model attacker_ai_v5_flash_v2_data/dqn_attacker_ai_v5_flash_v2_best.pt --eval --eval-q-trace-steps 30
"""
...
def build_training_maps(base_text: str) -> List[Tuple[str, np.ndarray]]:
    """固定マップの対称変換を使って、学習用マップ群を作る。

    値2/3/4（設置地点/攻撃側スポーン/防衛側スポーン）も一緒に変換されるため、
    各マップでルール上の意味と到達可能性を保ったまま方向暗記を抑えられる。
    """
    base = parse_grid(base_text)
    variants = [
        ("ORIGINAL", base.copy()),
        ("MIRROR_LEFT_RIGHT", np.fliplr(base).copy()),
        ("MIRROR_UP_DOWN", np.flipud(base).copy()),
        ("ROTATE_180", np.rot90(base, 2).copy()),
    ]

    # 念のため完全重複を除外する。
    unique: List[Tuple[str, np.ndarray]] = []
    seen: set[bytes] = set()
    for name, grid in variants:
        key = grid.tobytes()
        if key not in seen:
            seen.add(key)
            unique.append((name, grid))
    return unique
