"""AI Ver6 Recon Manual Multi-Map: 向きを維持した手作業の地形差分マップをエピソードごとに切り替える1vs2環境。
...
- 攻撃方向・スポーン・サイト位置を維持し、壁や開口部だけを変えた5マップをランダム選択
...
実行例:
    python train_attacker_ai_v6_recon_manual_maps.py --episodes 1000 --flash-model attacker_ai_v5_flash_v2_data/dqn_attacker_ai_v5_flash_v2_best.pt
    python train_attacker_ai_v6_recon_manual_maps.py --resume attacker_ai_v6_recon_manual_maps_data/training_state_latest.pt
    python train_attacker_ai_v6_recon_manual_maps.py --resume attacker_ai_v6_recon_manual_maps_data/training_state_latest.pt --eval
    python train_attacker_ai_v6_recon_manual_maps.py --resume attacker_ai_v6_recon_manual_maps_data/training_state_latest.pt --eval --debug-all
"""
...
MODEL_DIR = BASE_DIR / "attacker_ai_v6_recon_manual_maps_data"
LATEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v6_recon_manual_maps_latest.pt"
BEST_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v6_recon_manual_maps_best.pt"
FINAL_MODEL_PATH = MODEL_DIR / "dqn_attacker_ai_v6_recon_manual_maps_final.pt"
...
# 連打防止用クールダウン（tick単位）。使用直後は次のReconを投げられない。
RECON_COOLDOWN_TICKS = 3

# RECON_NEW_CELLは新規セル数に比例するが、広い範囲を一度に見た時に
# 報酬が過大化しないよう、加算対象セル数に上限をかける。
RECON_NEW_CELL_CAP_CELLS = 20
...
def build_training_maps(base_text: str) -> List[Tuple[str, np.ndarray]]:
    """攻撃方向を変えず、壁配置だけを調整した学習用マップ群を作る。

    2=サイト、3=攻撃側スポーン、4=防衛側スポーンは全マップで固定する。
    反転・回転は行わないため、既存方策の「サイトへ進む方向」を壊さずに
    遮蔽・通路・Reconの着弾価値だけを変化させる。
    """
    base = parse_grid(base_text)

    # ((row, col), new_value)。1=壁、0=通路。
    # 各差分はサイトやスポーンを変更せず、全重要地点の接続性を検証する。
    layouts: List[Tuple[str, List[Tuple[Position, int]]]] = [
        ("ORIGINAL", []),
        (
            "CENTRAL_COVER",
            [
    ...
    marker_positions = {
        value: set(zip(*np.where(base == value))) for value in (2, 3, 4)
    }
    variants: List[Tuple[str, np.ndarray]] = []

    for name, edits in layouts:
        grid = base.copy()
        for (row, col), value in edits:
            if not (0 <= row < grid.shape[0] and 0 <= col < grid.shape[1]):
                raise ValueError(f"{name}: 編集座標{(row, col)}がマップ外です")
            if grid[row, col] in (2, 3, 4):
                raise ValueError(f"{name}: サイト/スポーン{(row, col)}は変更できません")
            grid[row, col] = value

        for marker, expected in marker_positions.items():
            actual = set(zip(*np.where(grid == marker)))
            if actual != expected:
                raise ValueError(f"{name}: マーカー値{marker}の位置が変化しました")

        important = list(
            marker_positions[2] | marker_positions[3] | marker_positions[4]
        )
        if not important:
            raise ValueError(f"{name}: 重要地点がありません")
        reachable = bfs(important[0], grid)
        disconnected = [pos for pos in important if not np.isfinite(reachable[pos])]
        if disconnected:
            raise ValueError(
                f"{name}: 到達不能なサイト/スポーンがあります: {disconnected}"
            )

        variants.append((name, grid))
...
class Phase3OneVsTwoEnv:
    ...
    def __init__(self, seed: int = DEFAULT_SEED):
        ...
        self.training_maps = build_training_maps(NEW_MAZE_STR)
        if not self.training_maps:
            raise ValueError("学習用マップを1枚も生成できませんでした")
        self.map_name = self.training_maps[0][0]
        self.map_index = 0
        self._apply_map(self.training_maps[0][1])
    ...
    def _apply_map(self, grid: np.ndarray) -> None:
        """現在のエピソードで使うマップを適用し、依存キャッシュを再構築する。"""
        ...
        if not self.attacker_spawns:
            raise ValueError(f"{self.map_name}にアタッカースポーン値3がありません")
        if not self.plant_cells:
            raise ValueError(f"{self.map_name}に設置地点値2がありません")
    ...
    def select_episode_map(self) -> None:
        """エピソードごとに学習マップを均等確率で切り替える。"""
        self.map_index = self.rng.randrange(len(self.training_maps))
        self.map_name, grid = self.training_maps[self.map_index]
        self._apply_map(grid)
    ...
    def reset(
        ...
        # 毎エピソード、向きを維持した手作業マップ群から1枚をランダム選択する。
        self.select_episode_map()
    ...
    def has_los(
        self, a: Position, b: Position, grid: Optional[np.ndarray] = None
    ) -> bool:
    ...
    def recon_tactically_available(self) -> bool:
        return bool(
            self.attacker.alive and self.recon_charges > 0 and self.recon_cooldown <= 0
        )
    ...
    def recon_debug_metrics(self, action: int) -> Dict[str, object]:
        """Recon候補1方向の情報価値を、報酬を変更せず診断用に計算する。"""
        center = self.ability_target(action)
        available = self.recon_action_available(action)

        covered_cells: set[Position] = set()
        if available:
            for rr in range(center[0] - RECON_RADIUS, center[0] + RECON_RADIUS + 1):
                for cc in range(center[1] - RECON_RADIUS, center[1] + RECON_RADIUS + 1):
                    cell = (rr, cc)
                    if self.inside(cell) and self.walkable(cell):
                        covered_cells.add(cell)

        known_cells = covered_cells & self.recon_explored_cells
        new_cells = covered_cells - self.recon_explored_cells
        actual_targets = self.recon_targets_for_action(action) if available else []
        predicted_targets = (
            self.predicted_recon_target_count(action) if available else 0
        )
        ...
        return {
            ...
            "known_ratio": (len(known_cells) / total_cells if total_cells else 1.0),
            "actual_enemy_count": len(actual_targets),
            "actual_enemy_ids": [d.player_id for d in actual_targets],
            "predicted_enemy_count": int(predicted_targets),
        }

    def recon_debug_snapshot(self) -> List[Dict[str, object]]:
        """現在状態におけるRecon 8候補の診断値を返す。"""
        return [
            self.recon_debug_metrics(action)
    ...
    def tick_reveals(self) -> None:
        """Recon関連のtickベース状態（被発見時間・クールダウン）を1tick分進める。"""
        for defender in self.defenders:
            if defender.recon_reveal_ticks > 0:
                defender.recon_reveal_ticks -= 1
        if self.recon_cooldown > 0:
            self.recon_cooldown -= 1
    ...
    def handle_recon(self, action: int, add_reward) -> None:
        ...
        # 一度に広範囲を見ても報酬が過大化しないよう、加算セル数に上限をかける。
        newly_seen = 0
        for rr in range(center[0] - RECON_RADIUS, center[0] + RECON_RADIUS + 1):
            for cc in range(center[1] - RECON_RADIUS, center[1] + RECON_RADIUS + 1):
                cell = (rr, cc)
        ...
        if newly_seen > 0:
            capped_newly_seen = min(newly_seen, RECON_NEW_CELL_CAP_CELLS)
            add_reward(
                "RECON_NEW_CELL",
                capped_newly_seen * R_RECON_NEW_CELL,
            )
        ...
        self.recon_cooldown = RECON_COOLDOWN_TICKS
        ...
        # 新規セルも新規ターゲットも得られなかった場合は、同じ場所への
        # 無駄打ち（連打）とみなして追加の罰則を与える。
        if newly_seen == 0 and not fresh_targets:
            add_reward("RECON_REPEAT", R_RECON_REPEAT)
        ...
        self.pending_recon_outcomes.append(
            {
                ...
                # Reconの成果を4種類に分けて記録する。
                # 敵を検出したかどうかだけでReconの価値を決めないため、
                # 他3種はすべて発生時にFalseから始まり、evaluate側で更新する。
                "enemy_found": bool(affected_ids),
                "kill_helped": False,
                "plant_helped": False,
                "safe_reposition": False,
            }
        )
    ...
    def evaluate_recon_outcomes(self, add_reward) -> None:
        ...
        for event in self.pending_recon_outcomes:
            if (
                ...
                and not bool(event["plant_helped"])
            ):
                add_reward("RECON_ENABLE_PLANT", R_RECON_ENABLE_PLANT)
                event["plant_helped"] = True
            ...
            if self.tick >= int(event["expires_tick"]):
                if (
                    ...
                    and not bool(event["safe_reposition"])
                ):
                    add_reward(
                        "RECON_SAFE_REPOSITION",
                        R_RECON_SAFE_REPOSITION,
                    )
                    event["safe_reposition"] = True

                # R_RECON_POINTLESSは-1.0
                outcome_flags = (
                    event["enemy_found"],
                    event["kill_helped"],
                    event["plant_helped"],
                    event["safe_reposition"],
                )
                if not any(bool(flag) for flag in outcome_flags):
    ...
    def player_visible(self, observer: Player, target: Player) -> bool:
        if not observer.alive or not target.alive:
            return False
        if (
            observer.team == Team.ATTACKER
            and target.team == Team.DEFENDER
            and target.revealed
        ):
            return True
    ...
    def observation(self) -> np.ndarray:
        ...
        ability_actions = list(
            range(ACTION_ABILITY_UP_NEAR, ACTION_ABILITY_RIGHT_FAR + 1)
        )
        ...
        recon_features += [
            float(self.recon_action_available(action)) for action in ability_actions
        ]
        ...
        recon_features += [
            ...
            max(
                (defender.recon_reveal_ticks for defender in alive_defenders_for_recon),
                default=0,
            )
        ...
        for dr, dc in MOVE_DELTAS.values():
            ...
            out.append(
                float(
                    self.walkable(probe)
                    and any(
                        self.has_los(probe, d.pos, self.grid) for d in alive_defenders
                    )
                )
            )
    ...
    def handle_defuse(self, add_reward) -> None:
        ...
        if assigned is None or chebyshev(assigned.pos, self.spike_pos) > 1:
            self.clear_defuse()
            return
    ...
    def shoot_player(
        ...
        if shooter.team == Team.ATTACKER:
            ...
            for event in self.pending_recon_outcomes:
                if self.tick <= int(event["expires_tick"]) and not bool(
                    event["kill_helped"]
                ):
                    add_reward("RECON_FOLLOWUP_KILL", R_RECON_FOLLOWUP_KILL)
                    event["kill_helped"] = True
    ...
    def step(self, action: int):
        ...
        if action in MOVE_DELTAS and self.attacker.alive:
            ...
            ACTION_ABILITY_UP_NEAR <= action <= ACTION_ABILITY_RIGHT_FAR
            and self.attacker.alive
        ):
        ...
        info = {
            ...
            "defender_reveal_ticks": {
                defender.player_id: defender.recon_reveal_ticks
                for defender in self.defenders
            },
...
def load_base_weights_for_recon(
    ...
    for key, value in phase3_state.items():
        ...
        if key in current and current[key].shape == value.shape:
            current[key] = value.to(
                dtype=current[key].dtype, device=current[key].device
            )
            copied.append(key)
        else:
            skipped.append(key)
...
def evaluate(
    ...
    seed: int = 10_000,
    q_trace_steps: int = 30,
    debug_q: bool = False,
    debug_recon: bool = False,
) -> Dict[str, object]:
    ...
    map_episode_counts: Dict[str, int] = defaultdict(int)
    map_success_counts: Dict[str, int] = defaultdict(int)
    ...
    # 最初の評価エピソードで、実際に使用したReconだけを記録する。
    recon_use_trace: List[Dict[str, object]] = []
    pending_recon_debug: List[Dict[str, object]] = []
    ...
    for episode in range(1, episodes + 1):
        ...
        map_episode_counts[env.map_name] += 1
        ...
        while not done:
            ...
            # 通常評価では余分なQ値・Adapter解析を行わない。
            # Q値が必要なデバッグを明示した場合だけ追加計算する。
            q_values: Dict[str, float] = {}
            if debug_q or debug_recon:
            ...
            recon_debug_before_step: Optional[Dict[str, object]] = None
            if (
                debug_recon
                and episode == 1
                and ACTION_ABILITY_UP_NEAR <= action <= ACTION_ABILITY_RIGHT_FAR
            ):
                recon_debug_before_step = env.recon_debug_metrics(action)
                move_q_values = [
                    (
                        move_action,
                        float(q_values.get(ACTION_NAMES[move_action], float("-inf"))),
                    )
                    for move_action in (
                        ACTION_UP,
                        ACTION_DOWN,
                    ...
                    if move_action in env.valid_actions()
                ]
                best_move_action, best_move_q = max(
                    move_q_values, key=lambda pair: pair[1]
                )
                recon_debug_before_step.update(
                    {
                        "map_name": env.map_name,
                        "use_tick": env.tick + 1,
                        "attacker_pos": env.pos,
                        "enemy_pos_at_use": [d.pos for d in env.defenders],
                        "enemy_hp_at_use": [d.hp for d in env.defenders],
                        "enemy_alive_at_use": [d.alive for d in env.defenders],
                        "attacker_hp_at_use": env.hp,
                        "bomb_planted_at_use": env.bomb_planted,
                        "recon_charges_before": env.recon_charges,
                        "ability_q": float(
                            q_values.get(ACTION_NAMES[action], float("nan"))
                        ),
                        "best_move_action_name": ACTION_NAMES[best_move_action],
                        "best_move_q": best_move_q,
                    }
                )

            if debug_q and episode == 1 and len(q_trace) < q_trace_steps:
                q_trace.append(
                    {
                        "map_name": env.map_name,
                        "tick": env.tick,
            ...
            if debug_recon and episode == 1 and recon_debug_before_step is not None:
                recon_debug_before_step.update(
                    {
                        "result_tick": int(recon_debug_before_step["use_tick"]) + 5,
                        "plant_started": bool(env.planting or env.bomb_planted),
                        "plant_completed": bool(env.bomb_planted),
                    }
                )
                pending_recon_debug.append(recon_debug_before_step)

            if debug_recon and episode == 1:
                remaining_recon_debug: List[Dict[str, object]] = []
                for event in pending_recon_debug:
                    event["plant_started"] = bool(
                        event.get("plant_started", False)
                        or env.planting
                        or env.bomb_planted
                    )
                    event["plant_completed"] = bool(
                        event.get("plant_completed", False) or env.bomb_planted
                    )

                    if env.tick >= int(event["result_tick"]) or done:
                        hp_before = [int(v) for v in event["enemy_hp_at_use"]]
                        alive_before = [bool(v) for v in event["enemy_alive_at_use"]]
                        hp_after = [d.hp for d in env.defenders]
                        alive_after = [d.alive for d in env.defenders]
                        event["evaluated_tick"] = env.tick
                        event["damage_dealt_5ticks"] = sum(
                            max(before - after, 0)
                            for before, after in zip(hp_before, hp_after)
                        )
                        event["kills_5ticks"] = sum(
                            int(was_alive and not is_alive)
                            for was_alive, is_alive in zip(alive_before, alive_after)
                        )
                        event["attacker_damage_taken_5ticks"] = max(
                            int(event["attacker_hp_at_use"]) - env.hp,
                            0,
                        )
                        event["enemy_hp_after"] = hp_after
                        event["enemy_alive_after"] = alive_after
                        event["round_ended_before_5ticks"] = bool(
                            done and env.tick < int(event["result_tick"])
                        )
                        recon_use_trace.append(event)
                    else:
                        remaining_recon_debug.append(event)
                pending_recon_debug = remaining_recon_debug
        ...
        map_success_counts[env.map_name] += int(env.success)
    ...
    return {
        ...
        "reasons": reasons,
        "map_episode_counts": dict(sorted(map_episode_counts.items())),
        "map_win_rates": {
            name: map_success_counts.get(name, 0) / max(count, 1)
            for name, count in sorted(map_episode_counts.items())
        },
        ...
        "q_trace": q_trace,
        "recon_use_trace": recon_use_trace,
...
def print_map_evaluation(result: Dict[str, object]) -> None:
    print("[EVAL] map distribution / win rate")
    episode_counts = result.get("map_episode_counts", {})
    win_rates = result.get("map_win_rates", {})
    for map_name, count in episode_counts.items():
        print(
            f"  {map_name:24s} episodes={int(count):4d} "
            f"win_rate={float(win_rates.get(map_name, 0.0)):.3f}"
        )
...
def print_recon_use_trace(recon_use_trace: List[Dict[str, object]]) -> None:
    """最初の評価エピソードで実際に使用したReconと、5Tick後の結果だけを表示する。"""
    if not recon_use_trace:
        print("[RECON-USED] first evaluation episode: no Recon used")
        return

    print("[RECON-USED] first evaluation episode")
    for index, event in enumerate(recon_use_trace, start=1):
        print("=" * 64)
        print(f"RECON #{index}")
        print(f"  use_tick       : {int(event['use_tick'])}")
        print(f"  action         : {event['action_name']}")
        print(f"  attacker_pos   : {event['attacker_pos']}")
        print(f"  center         : {event['center']}")
        print(f"  enemy_pos      : {event['enemy_pos_at_use']}")
        print(f"  charges_before : {int(event['recon_charges_before'])}")
        print(f"  new_cells      : {int(event['new_cells'])}")
        print(f"  known_cells    : {int(event['known_cells'])}")
        print(f"  known_ratio    : {float(event['known_ratio']):.1%}")
        print(f"  predicted      : {int(event['predicted_enemy_count'])}")
        print(f"  actual         : {int(event['actual_enemy_count'])}")
        print(f"  ability_q      : {float(event['ability_q']):.3f}")
        print(
            f"  best_move_q    : {float(event['best_move_q']):.3f} "
            f"({event['best_move_action_name']})"
        )
        print("  --- result after 5 ticks ---")
        print(f"  evaluated_tick : {int(event['evaluated_tick'])}")
        print(f"  damage_dealt   : {int(event['damage_dealt_5ticks'])}")
        print(f"  kills          : {int(event['kills_5ticks'])}")
        print(f"  damage_taken   : {int(event['attacker_damage_taken_5ticks'])}")
        print(f"  plant_started  : {bool(event['plant_started'])}")
        print(f"  plant_completed: {bool(event['plant_completed'])}")
        print(f"  round_ended_early: " f"{bool(event['round_ended_before_5ticks'])}")
    print("=" * 64)
...
def train(
    ...
    selected_inputs = sum(
        x is not None for x in (resume, phase2_model, phase3_model, flash_model)
    )
    if selected_inputs > 1:
        raise ValueError(
            "--resume / --phase2-model / --phase3-model / --flash-modelは同時指定できません"
        )

    # Phase2モデルからの新規転移とPhase3 checkpoint再開では、
    # 完成済み方策を維持する低epsilonスケジュールを使う。
    use_transfer_epsilon = (
        phase2_model is not None
        or phase3_model is not None
        or flash_model is not None
    ...
    if eval_only:
        ...
        if debug_recon:
            print_recon_use_trace(result["recon_use_trace"])
        return
    ...
    print(
        "[MULTI-MAP] episode randomization:",
        [name for name, _ in env.training_maps],
    )
    print(
    ...
    try:
        for episode in range(
            ...
            # 評価は学習中に自動実行しない。
            # --eval（必要に応じて --debug-q / --debug-recon）を付けた
            # 別コマンドでのみ実行する。
...
def main() -> None:
    ...
    parser.add_argument(
        "--debug-q",
        action="store_true",
        help="--eval時にQ値トレースとAdapter診断を追加表示",
    )
    parser.add_argument(
        "--debug-recon",
        action="store_true",
        help="--eval時に最初の評価エピソードのRecon使用詳細を追加表示",
    )
    parser.add_argument(
        "--debug-all",
        action="store_true",
        help="--eval時にQ値・Adapter・Reconの全デバッグを追加表示",
    )
    ...
    if args.debug_all:
        args.debug_q = True
        args.debug_recon = True
    if (args.debug_q or args.debug_recon) and not args.eval:
        raise SystemExit(
            "--debug-q / --debug-recon / --debug-all は --eval と一緒に指定してください"
        )
